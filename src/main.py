        ...
        if battle_phase == BATTLE_PHASE_EXCHANGE:
            if battle_menu_selected_index in (BATTLE_MENU_INDEX_WHIP, BATTLE_MENU_INDEX_FLAME, BATTLE_MENU_INDEX_DANCE):
                ...
                if battle_attacking_enemy_index == -1:
                    # ヒロインの番：選択中のコマンドに応じてムチ（単体攻撃）／炎（全体攻撃）／マカダンス（自身の演出）を行う
...
def render_battle():
    ...
    dance_active = (battle_phase == BATTLE_PHASE_EXCHANGE and battle_menu_selected_index == BATTLE_MENU_INDEX_DANCE
                    and battle_attacking_enemy_index == -1 and battle_dance_phase == BATTLE_DANCE_PHASE_DANCE)
    ...
    if battle_back_img_raw and not dance_active:
        ...
        start_img_h = int(HEROINE_HEIGHT_M * BATTLE_HEROINE_FIRST_METER_TO_PIXEL)
        ...
        # （基準はバトルメインウィンドウが開ききった時点の下端＝band_bottom_full。開くアニメ中も移動しない）
        bottom_y_start = band_bottom_full + int(BATTLE_HEROINE_FIRST_FOCUS_M * BATTLE_HEROINE_FIRST_METER_TO_PIXEL)
        bottom_y_end   = band_bottom_full + int(BATTLE_HEROINE_LAST_FOCUS_M * BATTLE_HEROINE_LAST_METER_TO_PIXEL)
        ...
        dance_sinking = (battle_phase == BATTLE_PHASE_EXCHANGE and battle_menu_selected_index == BATTLE_MENU_INDEX_DANCE
                         and battle_attacking_enemy_index == -1 and battle_dance_phase == BATTLE_DANCE_PHASE_SINK)
        if dance_sinking:
        ...
        # ★ サムライ（仲間キャラ）の後ろ姿：ヒロインの1m右に、足元位置・待機モーションをヒロインと揃えて表示する（行動選択は未対応）
        if samurai_back_img_raw:
            sam_orig_w, sam_orig_h = samurai_back_img_raw.get_size()

            # 画像高さ：ヒロインと同じ meter_to_pixel スケールを、サムライの身長(SAMURAI_HEIGHT_M)に適用する
            meter_to_pixel = img_h / HEROINE_HEIGHT_M
            sam_img_h = max(1, int(SAMURAI_HEIGHT_M * meter_to_pixel))
            ...
            # 位置：ヒロインの1m右、足元の高さはヒロインと同じ
            sam_x = heroine_x + int(1.0 * meter_to_pixel)
            sam_img_rect = sam_img.get_rect(midbottom=(sam_x, bottom_y))
