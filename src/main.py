...
BATTLE_HEROINE_LAST_WINDOW_WIDTH_M  = 100 / 27  # ズームアウト後
...
def initialize():
    ...
    global result_win_img, result_win_img_raw, result_samurai_win_img, result_warrior_win_img, enemy_img_raw, dance_images_raw, voice_win_by_number, voice_samurai_win_by_number, voice_warrior_win_by_number
...
def render_result():
    ...
    if result_active_win_img:
        ...
        if arrived:
            ...
            if result_flashout_is_samurai or result_flashout_is_warrior:
                # サムライ・女戦士：待機モーションの拡縮は行わず、initialize()で算出したサイズのまま静止表示する
