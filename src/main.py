...
# ヒロインの勝利バストショット（bunny_walk_0.png）：ゲームウィンドウ幅(SCREEN_W)がワールド座標値で
...
def initialize():
    ...
    # ★ 勝利バストショット画像（ヒロイン）：bunny_walk_0.pngを、
...
def render_result():
    ...
    if result_active_win_img:
        ...
        if result_flashout_is_warrior:
            ...
            # ヒロイン（bunny_walk_0.png）：画像上端からRESULT_HEROINE_WIN_TOP_MARGIN_M分の位置が
            # ゲームウィンドウ上端(band_y)に一致するように配置する
            # （initialize()で一度だけ行ったスケールのまま、追加の拡縮は行わない）
            win_h = result_active_win_img.get_height()
