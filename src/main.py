...
voice_attack_list   = []    # 攻撃ボイス（複数候補。再生時にランダム選択）
...
#                       再生速度変更後のSoundのリストを返す（再生時にランダム選択する）
# ---------------------------------------------------------
...
def start_battle_turn():
    ...
    if attacker == -1:
        ...
        if battle_menu_selected_index == BATTLE_MENU_INDEX_FLAME:
            ...
            battle_whip_phase = BATTLE_WHIP_PHASE_APPROACH
            battle_whip_frame = 0
        if voice_attack_list:
            random.choice(voice_attack_list).play()
    else:
...
def initialize():
    ...
    global result_win_img, result_win_img_raw, enemy_img_raw, voice_win, voice_battle_start, voice_attack_list, voice_goblin_damaged
    ...
    voice_attack_list = load_attack_voices()
...
def process_input():
    ...
    for event in pygame.event.get():
        ...
        # Enterキー：ポーズ中は1フレームだけ処理を進める／攻撃選択サブウィンドウで実行 → 攻防ステートへ遷移（コマンドウィンドウは非表示）／リザルト画面からフィールドへ
