...
# 勝利ボイスの再生が終わった瞬間に再生する、胸元を強調する「ボヨヨン」拡大演出のパラメータ
# （バトルウィンドウ下端＝バストアップ画像下端を常に保ったまま、等方的に拡大→収束させる。
# 　画像高さは元々バトルウィンドウ高さに合わせてあるため、これより縮むと見栄えが崩れる
# 　→ 拡大方向にのみ一度だけ膨らみ、最小スケール＝元のサイズへなめらかに戻る。振動はさせない）
# 振動波 = sin(π × 進行度)  ※ 進行度 0→1 に対して 0→1→0 と単峰に変化する（拡大・収束を１往復のみ）
# スケール = 1.0 + 振幅 × 振動波
# → 開始・終了ともに等倍（最小スケール）。中間地点で最大膨張率に達したのち、なめらかに元へ戻る
RESULT_BOUNCE_FRAMES       = 25   # 演出の継続フレーム数（最大膨張に達して元のスケールに戻るまでの所要フレーム数）
RESULT_BOUNCE_AMPLITUDE    = 2    # 最大膨張率（最大スケールは 1.0 + この値となる）
...
RESULT_WIN_BGM_START_SEC   = 147.2  # 勝利BGM代用：再生開始時にbattle.mp3を再生開始する秒数
...
result_bounce_frame      = 0   # 勝利ボイスの再生が終わった瞬間からの拡大演出の経過フレーム数
...
# また、Enterキーを押し続けた場合はキーリピートで連続して1フレームずつ進める
# （最初は PAUSE_STEP_REPEAT_DELAY_SEC 秒後に1回、以後は PAUSE_STEP_REPEAT_INTERVAL_SEC 秒間隔で繰り返す）
PAUSE_STEP_REPEAT_DELAY_SEC    = 0.6  # 押し続けてから最初のリピートが発生するまでの秒数
PAUSE_STEP_REPEAT_INTERVAL_SEC = 0.1  # 2回目以降のリピート間隔（秒）
...
pause_enter_held       = False  # Enterキーが押され続けているか（リピート判定用）
pause_enter_hold_frame = 0      # Enterキーを押し続けているフレーム数（リピート判定用カウンタ）
pause_enter_repeated   = False  # 押し続けている間に既に1回以上リピートが発生したか（遅延／間隔の切り替え用）
...
VOICE_ATTACK_DIR = os.path.join(BASE_DIR, "..", "assets", "sound", "voices")
...
result_win_voice_frames = 0  # 勝利ボイスの再生時間（フレーム数換算。再生終了の瞬間を判定するため初期化時に算出する）
...
def load_attack_voices():
    files = [f for f in os.listdir(VOICE_ATTACK_DIR)
    ...
    voices_by_number = {}
    for fname in files:
        voice = load_sound_at_speed(os.path.join(VOICE_ATTACK_DIR, fname), VOICE_ATTACK_SPEED)
...
def initialize():
    ...
    global result_win_img, result_win_img_raw, enemy_img_raw, voice_win, result_win_voice_frames
    global voice_battle_start, voice_attack_by_number, voice_goblin_damaged
    ...
    result_win_voice_frames = max(1, round(voice_win.get_length() * FIXED_FPS))
...
def process_input():
    ...
    global pause_enter_held, pause_enter_hold_frame, pause_enter_repeated
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN and event.key == pygame.K_p:
            ...
            pause_step_requested = False
            pause_enter_held       = False
            pause_enter_hold_frame = 0
            pause_enter_repeated   = False
            if is_paused:
                pygame.mixer.pause()
                pygame.mixer.music.pause()
            else:
        ...
        # Enterキー：ポーズ中は1フレームだけ処理を進める（押し続けるとキーリピートでも進む）
        # ／攻撃選択サブウィンドウで実行 → 攻防ステートへ遷移（コマンドウィンドウは非表示）／リザルト画面からフィールドへ
        if event.type == pygame.KEYDOWN and event.key == pygame.K_RETURN:
            if is_paused:
                pause_step_requested = True
                pause_enter_held       = True
                pause_enter_hold_frame = 0
                pause_enter_repeated   = False
        ...
        # Enterキーを離した瞬間：キーリピートの判定状態をリセットする
        if event.type == pygame.KEYUP and event.key == pygame.K_RETURN:
            pause_enter_held       = False
            pause_enter_hold_frame = 0
            pause_enter_repeated   = False

    # ポーズ中、Enterキーを押し続けている間のキーリピート処理
    # （最初は PAUSE_STEP_REPEAT_DELAY_FRAMES 経過後に1回、以後は PAUSE_STEP_REPEAT_INTERVAL_FRAMES ごとに1フレームずつ進める）
    if is_paused and pause_enter_held:
        pause_enter_hold_frame += 1
        repeat_threshold = (PAUSE_STEP_REPEAT_INTERVAL_FRAMES if pause_enter_repeated
                            else PAUSE_STEP_REPEAT_DELAY_FRAMES)
        if pause_enter_hold_frame >= repeat_threshold:
            pause_step_requested = True
            pause_enter_hold_frame = 0
            pause_enter_repeated = True
...
def update(dt):
    ...
    if game_state == STATE_RESULT:
        ...
        if result_text_frame == 0 and voice_win:
            voice_win.play()
        # 勝利ボイスの再生が終わった瞬間から、胸元を強調する「ボヨヨン」拡大演出を開始する
        # （後続のテキスト表示等の進行をブロックしない演出専用カウンタとして並行して進める）
        if result_text_frame >= result_win_voice_frames and result_bounce_frame < RESULT_BOUNCE_FRAMES:
            result_bounce_frame += 1
...
def render_result():
    ...
    if result_win_img:
        ...
        if arrived:
            ...
            # ★ 勝利ボイスの再生が終わった瞬間：胸元を強調するように、ウィンドウ下端＝画像下端を
            # 保ったまま等方的に一度だけ膨らんで元のスケールに戻る「ボヨヨン」拡大演出を適用する
            # （元のサイズが最小スケールとなり、それ以上縮むことはない）
            display_img = result_win_img
            display_rect = img_rect
            if RESULT_BOUNCE_FRAMES > 0 and result_bounce_frame < RESULT_BOUNCE_FRAMES:
                bounce_t = result_bounce_frame / RESULT_BOUNCE_FRAMES
                bounce_wave = math.sin(math.pi * bounce_t)  # 0→1→0 と単峰に変化する（往復１回のみ）
                scale = 1.0 + RESULT_BOUNCE_AMPLITUDE * bounce_wave
                bounced_w = max(1, round(win_w * scale))
                bounced_h = max(1, round(result_win_img.get_height() * scale))
                display_img = pygame.transform.smoothscale(result_win_img, (bounced_w, bounced_h))
