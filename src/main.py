                        ...
                        if battle_samurai_menu_selected_index == SAMURAI_MENU_INDEX_SHINGANKEN:
                            ...
                            if battle_samurai_menu_selected_index == SAMURAI_MENU_INDEX_SONICKEN:
                                ...
                                if battle_samurai_whip_phase == BATTLE_WHIP_PHASE_APPROACH:
                                    ...
                                    if battle_sonicken_slash_frame == last_slash_start + (BATTLE_SLASH_DRAW_FRAMES - 1):
                                        # 最後の斬撃の軌跡先端到達時にダメージを一括適用
                                    ...
                                    if (not all(enemy_defeated)
                                            and battle_sonicken_slash_frame >= last_slash_start + SONICKEN_END_DELAY_FRAMES):
                                        battle_samurai_target_enemy_index = find_alive_enemy_index(battle_samurai_target_enemy_index, 1)
                                        advance_battle_turn()
                            else:
