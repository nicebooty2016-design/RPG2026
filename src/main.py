...
BATTLE_SLASH_BASE_WIDTH_M = 0.08     # 二等辺三角形の底辺（軌跡の先端側）の長さ（ワールド座標系メートル。エフェクト発生時の敵の表示スケールに合わせてENEMY_GOBLIN_LAST_METER_TO_PIXELでピクセル変換する）
...
SHINGANKEN_WAIT_FRAMES = 16            # 開始から1回目の斬撃が始まるまでのフレーム数（その場で待機）
SHINGANKEN_SLASH_INTERVAL_FRAMES = 8   # 各斬撃の開始間隔フレーム数
SHINGANKEN_END_DELAY_FRAMES = 16       # 最後（7回目）の斬撃開始から攻撃終了までのフレーム数
...
def get_samurai_base_position():
    ...
    if battle_focus_samurai:
        ...
        sam_base_x = SCREEN_W // 2 + int(1.0 * BATTLE_HEROINE_LAST_METER_TO_PIXEL)
            ...
            if i == battle_shingan_target_enemy_index:
                ...
                for slash_i in range(SHINGANKEN_SLASH_COUNT):
                    if battle_shingan_slash_frames[slash_i] < BATTLE_SLASH_TOTAL_FRAMES:
                        if shingan_top_y is None:
                            shingan_top_y = get_art_top_y("goblin_idle.png", ENEMY_GOBLIN_HEIGHT_M, enemy_img_bottom_y, final_img_h)
                        draw_shingan_slash(enemy_rect, shingan_top_y, battle_shingan_slash_frames[slash_i], battle_shingan_slash_angles[slash_i])
