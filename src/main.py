...
BATTLE_WHIP_TARGET_SCALE    = 0.5     # 最接近時の画像高さ（画面高さの何倍。ズームアウト後の踊り子画像高さより小さい値を指定）
BATTLE_WHIP_TARGET_GROUND_Y_FROM_BOTTOM_RATIO = 0.1  # 最接近時の踊り子の足元位置：画面下端から画面高さの何倍の位置か（敵とは独立して指定）
...
def start_battle():
    ...
    global dancer_whip_trail_key, samurai_whip_trail_key, warrior_whip_trail_key
                                ...
                                if battle_samurai_whip_phase == BATTLE_WHIP_PHASE_APPROACH:
                                    ...
                                    if battle_samurai_whip_frame >= BATTLE_WHIP_FLASH_FRAMES:
                                        ...
                                        if all(enemy_defeated):
                                            ...
                                            sword_target_img_h = int(SCREEN_H * BATTLE_WHIP_TARGET_SCALE * (SAMURAI_HEIGHT_M / DANCER_HEIGHT_M))
                                            enter_result_state(samurai_finish=True,
                        ...
                        if battle_warrior_whip_phase == BATTLE_WHIP_PHASE_APPROACH:
                            ...
                            if battle_warrior_whip_frame >= BATTLE_WHIP_FLASH_FRAMES:
                                ...
                                if all(enemy_defeated):
                                    ...
                                    axe_target_img_h = int(SCREEN_H * BATTLE_WHIP_TARGET_SCALE * (WARRIOR_HEIGHT_M / DANCER_HEIGHT_M))
    ...
    if battle_back_img_raw:
        ...
        if sam_back_img_raw:
            ...
            if sword_active:
                ...
                elif battle_samurai_whip_phase == BATTLE_WHIP_PHASE_RETURN:  # RETURN：ワープ位置→元位置
                    raw_t = (battle_samurai_whip_frame / BATTLE_WHIP_APPROACH_FRAMES) if BATTLE_WHIP_APPROACH_FRAMES > 0 else 1.0
                    t = 1.0 - (1.0 - raw_t) ** 2
                ...
                else:  # FLASH（斬撃フェーズ）：battle_samurai_whip_frameは0で固定のためRETURN補間は使えない。元位置に固定する
                    sam_x        = sam_base_x
            ...
            def draw_samurai_sprite():
                ...
                if sonicken_active and battle_samurai_whip_phase == BATTLE_WHIP_PHASE_RETURN:
                    ...
                    if sam_trail_b is not None:
                        blit_dancer_trail_image(sam_back_img_raw, sam_trail_b, BATTLE_WHIP_TRAIL_ALPHA_2, clip_rect)
                    if sam_trail_a is not None:
                        blit_dancer_trail_image(sam_back_img_raw, sam_trail_a, BATTLE_WHIP_TRAIL_ALPHA_1, clip_rect)
            ...
            if warrior_back_img_raw:
                ...
                if sister_back_img_raw:
                    ...
                    if sister_active:
                        ...
                        sis_target_bottom_y = SCREEN_H - int(SCREEN_H * BATTLE_WHIP_TARGET_GROUND_Y_FROM_BOTTOM_RATIO)
                        sis_target_img_h = int(SCREEN_H * BATTLE_WHIP_TARGET_SCALE * (SISTER_HEIGHT_M / DANCER_HEIGHT_M))
                ...
                if kunoichi_back_img_raw:
                    ...
                    if kunoichi_active:
                        ...
                        kun_target_bottom_y = SCREEN_H - int(SCREEN_H * BATTLE_WHIP_TARGET_GROUND_Y_FROM_BOTTOM_RATIO)
                        kun_target_img_h = int(SCREEN_H * BATTLE_WHIP_TARGET_SCALE * (KUNOICHI_HEIGHT_M / DANCER_HEIGHT_M))
                ...
                if wizard_back_img_raw:
                    ...
                    if wizard_active:
                        ...
                        wiz_target_bottom_y = SCREEN_H - int(SCREEN_H * BATTLE_WHIP_TARGET_GROUND_Y_FROM_BOTTOM_RATIO)
                        wiz_target_img_h = int(SCREEN_H * BATTLE_WHIP_TARGET_SCALE * (WIZARD_HEIGHT_M / DANCER_HEIGHT_M))
                ...
                if fighter_back_img_raw:
                    ...
                    if fighter_active:
                        ...
                        fig_target_bottom_y = SCREEN_H - int(SCREEN_H * BATTLE_WHIP_TARGET_GROUND_Y_FROM_BOTTOM_RATIO)
                        fig_target_img_h = int(SCREEN_H * BATTLE_WHIP_TARGET_SCALE * (FIGHTER_HEIGHT_M / DANCER_HEIGHT_M))
