...
VOICE_BATTLE_START_SPEED = 1.6  # エンカウント時かけ声の再生速度倍率（候補とも一律）
...
# 攻撃選択サブウィンドウ
BATTLE_MENU_OPTIONS = ["ムチ", "ドレイン", "炎", "マカダンス"]  # 攻撃手段の選択肢（上から順に表示）

BATTLE_MENU_WIDTH  = 180  # サブウィンドウの幅（px）
BATTLE_MENU_HEIGHT = 176  # サブウィンドウの高さ（px）
...
BATTLE_MENU_INDEX_DANCE = 3  # BATTLE_MENU_OPTIONS内の「マカダンス」のインデックス

# サムライの行動選択肢（ヒロインの選択後に表示。現状は剣のみ）
SAMURAI_MENU_OPTIONS = ["剣"]
...
BATTLE_DANCE_WINDOW_COLOR = (252, 15, 192)   # ダンス中のバトルウィンドウ色（ショッキングピンク）
BATTLE_DANCE_IMAGE_INTERVAL_FRAMES = 0.6       # ダンス画像（bunny_dance_0フォルダ）が切り替わる間隔のフレーム数（ループ再生）
...
def process_input():
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN and event.key == pygame.K_UP and not is_paused:
            if game_state == STATE_BATTLE and heroine_zoomout_frame >= BATTLE_HEROINE_ZOOMOUT_FRAMES:
                if battle_phase == BATTLE_PHASE_COMMAND_HEROINE:
                    ...
                    battle_samurai_menu_selected_index = (battle_samurai_menu_selected_index - 1) % len(SAMURAI_MENU_OPTIONS)
        ...
        # ムチ／剣選択中：左右キーで攻撃対象の敵を変更（ズームアウト完了後・コマンド選択中のみ。撃破済みの敵は選択をスキップする。システムポーズ中は無効）
        ...
        if event.type == pygame.KEYDOWN and event.key == pygame.K_RETURN and not is_paused:
            if (game_state == STATE_BATTLE and heroine_zoomout_frame >= BATTLE_HEROINE_ZOOMOUT_FRAMES
                    and battle_phase == BATTLE_PHASE_COMMAND_HEROINE):
                # ヒロインの行動が決まったら、サムライの行動選択へ進む
                battle_phase = BATTLE_PHASE_COMMAND_SAMURAI
                battle_samurai_menu_selected_index = 0
                battle_samurai_target_enemy_index  = find_alive_enemy_index(-1, 1)
                    ...
                    and battle_phase == BATTLE_PHASE_COMMAND_SAMURAI):
                # サムライの行動が決まったら、攻防ステートへ遷移する
        ...
        # Spaceキー：ステータスウィンドウを閉じてフィールドモードへ戻る／サムライの行動選択をキャンセルしてヒロインの選択へ戻る（システムポーズ中は無効）
                        ...
                        if battle_samurai_whip_phase == BATTLE_WHIP_PHASE_APPROACH:
                            ...
                            if battle_samurai_whip_frame >= BATTLE_WHIP_FLASH_FRAMES:
                                ...
                                if all(enemy_defeated):
                                    # 最後の敵を撃破：サムライは画面中央へ接近していないため、表示位置の上書きは不要
                                    enter_result_state()
                                else:
...
def blit_heroine_trail_image(raw_img, pos, alpha):
    ...
    trail_rect = trail_img.get_rect(midbottom=(x, boty))
...
def render_battle():
    ...
    heroine_base_t_zoom = min(1.0, heroine_zoomout_frame / BATTLE_HEROINE_ZOOMOUT_FRAMES) \
                          if BATTLE_HEROINE_ZOOMOUT_FRAMES > 0 else 1.0
    heroine_base_t_eased = 1.0 - (1.0 - heroine_base_t_zoom) ** 2
    heroine_base_start_img_h = int(HEROINE_HEIGHT_M * BATTLE_HEROINE_FIRST_METER_TO_PIXEL)
    heroine_base_end_img_h   = int(HEROINE_HEIGHT_M * BATTLE_HEROINE_LAST_METER_TO_PIXEL)
    heroine_base_img_h = max(1, int(heroine_base_start_img_h + (heroine_base_end_img_h - heroine_base_start_img_h) * heroine_base_t_eased))
    heroine_base_bottom_y_start = band_bottom_full + int(BATTLE_HEROINE_FIRST_FOCUS_M * BATTLE_HEROINE_FIRST_METER_TO_PIXEL)
    heroine_base_bottom_y_end   = band_bottom_full + int(BATTLE_HEROINE_LAST_FOCUS_M * BATTLE_HEROINE_LAST_METER_TO_PIXEL)
    heroine_base_bottom_y = int(heroine_base_bottom_y_start + (heroine_base_bottom_y_end - heroine_base_bottom_y_start) * heroine_base_t_eased)

    # ★ サムライの基準位置（ヒロインの1m右、足元位置・スケールはこの時点のヒロインに合わせる）
    sam_meter_to_pixel = heroine_base_img_h / HEROINE_HEIGHT_M
    sam_base_x = SCREEN_W // 2 + int(1.0 * sam_meter_to_pixel)
    sam_base_bottom_y = heroine_base_bottom_y
        ...
        for i, x_ratio in enumerate(ENEMY_X_RATIOS):
            ...
            if is_attacking:
                ...
                if battle_enemy_attack_target == -2:
                    ...
                    # ヒロインへの攻撃：画面中央（ヒロインの基準位置）に向けて接近する
    ...
    # ★ ヒロイン後ろ姿の描画（マカダンスのダンス中は画面下に消えているため描画しない）
    if battle_back_img_raw and not dance_active:
        ...
        if idle_active:
            idle_w = max(1, int(img.get_width()  * idle_sx))
            idle_h = max(1, int(img.get_height() * idle_sy))
        ...
        if samurai_back_img_raw:
            ...
            sam_img = pygame.transform.smoothscale(samurai_back_img_raw, (sam_img_w, sam_img_h))

            # ④ 待機モーション：ヒロインと同じ縦横逆位相スクワッシュ（剣攻撃中は適用しない）
            if idle_active and not sword_active:
                sam_idle_w = max(1, int(sam_img.get_width()  * idle_sx))
                sam_idle_h = max(1, int(sam_img.get_height() * idle_sy))
                sam_img = pygame.transform.smoothscale(sam_img, (sam_idle_w, sam_idle_h))
            ...
            if samurai_flash_active:
                blink_half = max(1, BATTLE_WHIP_FLASH_BLINK_PERIOD_FRAMES // 2)
                if (battle_enemy_attack_frame // blink_half) % 2 == 0:
                    sam_img = sam_img.copy()
                    sam_img.fill(BATTLE_WHIP_FLASH_COLOR, special_flags=pygame.BLEND_RGB_ADD)

            sam_img_rect = sam_img.get_rect(midbottom=(sam_x, sam_bottom_y))
            ...
            def draw_samurai_sprite():
                ...
                if sword_active and battle_samurai_whip_phase == BATTLE_WHIP_PHASE_RETURN:
                    ...
                    if sam_trail_b is not None:
                        blit_heroine_trail_image(samurai_back_img_raw, sam_trail_b, BATTLE_WHIP_TRAIL_ALPHA_2)
                    if sam_trail_a is not None:
                        blit_heroine_trail_image(samurai_back_img_raw, sam_trail_a, BATTLE_WHIP_TRAIL_ALPHA_1)
                    screen.blit(sam_img, sam_img_rect)
                else:
                    # 近づくとき（および通常時）：現在画像 → A → B の順で重ねる
                    screen.blit(sam_img, sam_img_rect)
                    if sam_trail_a is not None:
...
def render_result():
    ...
    if battle_flashout_frame < BATTLE_FLASHOUT_FRAMES:
        ...
        if battle_back_img_raw:
            ...
            alpha_img_rect = img.get_rect(midbottom=(heroine_x, boty))
    ...
    if result_win_img:
        ...
        if arrived:
            ...
            display_img = pygame.transform.smoothscale(result_win_img, (idle_w, idle_h))
