...
# 踊り子の勝利バストショット（bunny_front.png）：ゲームウィンドウ幅(SCREEN_W)がワールド座標値で
# 何mに該当するかを指定する。ここから算出したDANCER_HEIGHT_M分の高さに一度だけスケールし、
# 以後リザルト中の追加の拡縮（待機モーションの拡縮等）は行わない
RESULT_DANCER_WIN_WIDTH_M = 1.0
# 表示位置：バストショット画像の上端から何mの位置を、ゲームウィンドウ上端に一致させるか
RESULT_DANCER_WIN_TOP_MARGIN_M = 0.15
...
# 音量
BGM_BATTLE_VOLUME = 0.20  # バトルBGM音量（0.0～1.0）
BGM_BATTLE2_VOLUME = 0.1  # 戦闘曲（battle2.mp3）の音量（0.0～1.0）
...
VOICE_DANCER_DAMAGED_VOLUME = 1.0  # 踊り子被弾やられボイスの音量（0.0～1.0）
...
# BATTLE_MENU_HEIGHT は廃止：render_battle_menu() で選択肢数に応じて動的に算出する
...
BATTLE_MENU_BORDER_RADIUS = int(6 * FONT_SCALE)        # 枠の角の丸み（px）。0で角ばった矩形
...
BATTLE_MENU_LINE_HEIGHT    = int(26 * FONT_SCALE)  # 選択肢1行あたりの高さ（px）
# ウィンドウ高さはrender_battle_menu()内で選択肢数に応じて動的に算出する（固定値は使わない）
...
BATTLE_ATTACK_APPROACH_FRAMES = 15      # 敵に接近するまでのフレーム数（後退も同じフレーム数で行う）

# 音速剣：移動先の足元Y位置（BATTLE_MEMBER_LAST_GROUND_Y_FROM_BOTTOM_RATIOと同じ定義方式）
# 移動方向はサムライ足元→対象敵足元のベクトル。接近後は瞬時にワープ先へ飛び、元位置へ帰還してターン終了
SONICKEN_APPROACH_Y_FROM_BOTTOM_RATIO = 0.5   # 接近目標の足元Y位置比率
SONICKEN_WARP_Y_FROM_BOTTOM_RATIO     = -0.5  # ワープ先の足元Y位置比率（元位置より下）

# ムチ：接近・後退中の残像（モーションブラー風の演出。数フレーム前の踊り子画像を半透明で重ねて表示する）
BATTLE_ATTACK_TRAIL_OFFSET_1 = 5         # 残像(A)：何フレーム前の位置を表示するか
BATTLE_ATTACK_TRAIL_OFFSET_2 = 10         # 残像(B)：何フレーム前の位置を表示するか
BATTLE_ATTACK_TRAIL_ALPHA_1 = 0.7        # 残像(A)の不透明度
BATTLE_ATTACK_TRAIL_ALPHA_2 = 0.3        # 残像(B)の不透明度
BATTLE_ATTACK_TRAIL_MIN_OFFSET_PX = 2    # 残像を表示する最小の位置差（ピクセル）。これ未満なら現在位置とほぼ同じとみなして表示しない

# ムチ：最接近直後のダメージ演出（攻撃ボイス再生 → 数フレーム後に敵を白色点滅 → 点滅終了直後に後退開始）
BATTLE_ATTACK_DAMAGE_DELAY_FRAMES = 10        # 最接近（攻撃ボイス再生開始）から敵の白色点滅開始までのフレーム数
BATTLE_ATTACK_FLASH_FRAMES = 10               # 敵の白色点滅の継続フレーム数（経過後、即座に後退を開始する）
BATTLE_DANCE_BLINK_PERIOD_FRAMES = 4   # 点滅1周期のフレーム数
...
BATTLE_SLASH_BASE_WIDTH_M = 0.03     # 二等辺三角形の底辺（軌跡の終端側）の長さ（ワールド座標系メートル。エフェクト発生時の敵の表示スケールに合わせてENEMY_GOBLIN_LAST_METER_TO_PIXELでピクセル変換する）
BATTLE_SLASH_DRAW_FRAMES  = 3        # 右上から左下まで軌跡が伸びるフレーム数
BATTLE_SLASH_HOLD_FRAMES  = 12        # 軌跡を維持するフレーム数
...
SHINGANKEN_SLASH_HOLD_FRAMES  = BATTLE_SLASH_HOLD_FRAMES * 2  # 心眼剣の斬撃を維持するフレーム数（通常の2倍）
SHINGANKEN_SLASH_TOTAL_FRAMES = BATTLE_SLASH_DRAW_FRAMES + SHINGANKEN_SLASH_HOLD_FRAMES + BATTLE_SLASH_ERASE_FRAMES
...
# マカダンス（味方バフ）演出パラメータ：踊り子が画面下に消える → バトルウィンドウがピンクに染まりセクシーダンスを再生
# （仲間がいない現状はバフ対象が存在しないため、演出終了後はそのまま次の番へ進む）
BATTLE_DANCE_SINK_FRAMES = 10                # 待機モーションの踊り子が画面下に消えるまでのフレーム数
...
# 敵の攻撃演出パラメータ：踊り子に接近 → 最接近で停止 → 元の位置へ後退（流れはムチ演出と同じ4ステート構成を共有する）
...
DANCER_MAX_HP  = 200   # 踊り子の最大HP（現在HPは戦闘をまたいで引き継ぐ。回復はしない）
...
SHINGANKEN_SLASH_COUNT = 1             # 斬撃の総数（水平1本）
...
SONICKEN_SLASH_COUNT = 4              # 斬撃の総数
SONICKEN_SLASH_INTERVAL_FRAMES = 8    # 各斬撃の開始間隔フレーム数
SONICKEN_END_DELAY_FRAMES = 20        # 最後の斬撃開始から攻撃終了までのフレーム数
SONICKEN_DAMAGE = 300                 # 与えるダメージ（最後の斬撃先端到達時に一括適用）
BATTLE_FLAME_DAMAGE_MIN = 10   # 炎で敵1体ごとに与えるダメージの最小値（全体攻撃だが各々個別に抽選する）
BATTLE_FLAME_DAMAGE_MAX = 40   # 炎で敵1体ごとに与えるダメージの最大値
BATTLE_ENEMY_ATTACK_DAMAGE_MIN = 10   # 敵の攻撃で踊り子／サムライが受けるダメージの最小値
BATTLE_ENEMY_ATTACK_DAMAGE_MAX = 30   # 敵の攻撃で踊り子／サムライが受けるダメージの最大値
...
battle_focus_character = -1  # ズームアウトで注視するキャラクター（-1=踊り子, -2=サムライ, -3=女戦士。エンカウント毎にランダム決定）
member_idle_frame       = 0
...
result_text_frame        = 0
result_flashout_dancer_override = None  # フラッシュアウト中の踊り子表示位置・スケールの上書き指定（(x, 足元y, 画像高さ) または None）
result_flashout_samurai_override = None  # フラッシュアウト中のサムライ表示位置・スケールの上書き指定（(x, 足元y, 画像高さ) または None）
result_flashout_is_samurai = False  # フラッシュアウトで踊り子の代わりにサムライを表示するか（サムライがトドメを刺した場合）
result_flashout_warrior_override = None  # フラッシュアウト中の女戦士表示位置・スケールの上書き指定（(x, 足元y, 画像高さ) または None）
result_flashout_is_warrior = False  # フラッシュアウトで踊り子の代わりに女戦士（または新キャラ）を表示するか
result_flashout_killed_enemy_index = -1  # フラッシュアウト中に描画する敵のインデックス（-1=描画しない、それ以外=そのインデックスの敵のみ描画）
result_flashout_active_char = -1  # トドメを刺したキャラのID（-1=踊り子, -2=サムライ, -3=女戦士, -4〜-7=新キャラ）
...
# AIユーザモード：踊り子の行動選択 → マカダンスはそのバトル中1回のみ選択可能（使用済みなら以後は選択肢から除外する）
dancer_macadance_used = False

# HP：踊り子・サムライ他の現在HPは戦闘終了時に全回復する。敵の現在HPはエンカウント毎に最大HPへリセットされる
dancer_hp  = DANCER_MAX_HP
...
dancer_damage_anim_old_hp = DANCER_MAX_HP
dancer_damage_anim_new_hp = DANCER_MAX_HP
dancer_damage_anim_frame  = DAMAGE_ANIM_DONE_FRAME
dancer_damage_anim_flash_color = DAMAGE_FLASH_COLOR_ENEMY_ATTACK
...
# 戦闘内ステート：踊り子・サムライ・女戦士の行動選択（順序はbattle_command_phase_order参照） → 攻防演出
BATTLE_PHASE_COMMAND_DANCER  = 0  # 踊り子の行動選択中（コマンドウィンドウ表示）
...
battle_phase = BATTLE_PHASE_COMMAND_DANCER
battle_exchange_frame = 0

# 行動選択順の基本サイクル：エンカウント時の注視キャラに応じて、この並びを回転させたものが
# battle_command_phase_order になる（例：女戦士注視→女戦士,踊り子,サムライ／踊り子注視→踊り子,サムライ,女戦士）
BATTLE_COMMAND_PHASE_CYCLE = [BATTLE_PHASE_COMMAND_DANCER, BATTLE_PHASE_COMMAND_SAMURAI, BATTLE_PHASE_COMMAND_WARRIOR, BATTLE_PHASE_COMMAND_SISTER, BATTLE_PHASE_COMMAND_KUNOICHI, BATTLE_PHASE_COMMAND_WIZARD, BATTLE_PHASE_COMMAND_FIGHTER]
BATTLE_FOCUS_CHARACTER_TO_COMMAND_PHASE = {-1: BATTLE_PHASE_COMMAND_DANCER, -2: BATTLE_PHASE_COMMAND_SAMURAI, -3: BATTLE_PHASE_COMMAND_WARRIOR, -4: BATTLE_PHASE_COMMAND_SISTER, -5: BATTLE_PHASE_COMMAND_KUNOICHI, -6: BATTLE_PHASE_COMMAND_WIZARD, -7: BATTLE_PHASE_COMMAND_FIGHTER}
BATTLE_COMMAND_PHASE_TO_CHARACTER = {BATTLE_PHASE_COMMAND_DANCER: -1, BATTLE_PHASE_COMMAND_SAMURAI: -2, BATTLE_PHASE_COMMAND_WARRIOR: -3, BATTLE_PHASE_COMMAND_SISTER: -4, BATTLE_PHASE_COMMAND_KUNOICHI: -5, BATTLE_PHASE_COMMAND_WIZARD: -6, BATTLE_PHASE_COMMAND_FIGHTER: -7}
...
BATTLE_ATTACK_PHASE_APPROACH    = 0  # 敵に接近中
BATTLE_ATTACK_PHASE_DAMAGE_WAIT = 1  # 最接近後、攻撃ボイス再生から白色点滅開始までの待機中
BATTLE_ATTACK_PHASE_FLASH       = 2  # ダメージ演出（敵の白色点滅）中
BATTLE_ATTACK_PHASE_RETURN      = 3  # 元の位置へ後退中
battle_whip_phase = BATTLE_ATTACK_PHASE_APPROACH
battle_whip_frame = 0

# サムライの剣（近接攻撃）演出ステート：ムチと同じ4ステートをそのまま流用する
battle_samurai_attack_phase = BATTLE_ATTACK_PHASE_APPROACH
battle_samurai_attack_frame = 0

# 女戦士の斧（近接攻撃）演出ステート：ムチと同じ4ステートをそのまま流用する
battle_warrior_attack_phase = BATTLE_ATTACK_PHASE_APPROACH
battle_warrior_attack_frame = 0

# シスター・くノ一・魔法使い・武道家の攻撃演出ステート：いずれもムチと同じ4ステートを流用する
battle_sister_attack_phase   = BATTLE_ATTACK_PHASE_APPROACH
battle_sister_attack_frame   = 0
battle_kunoichi_attack_phase = BATTLE_ATTACK_PHASE_APPROACH
battle_kunoichi_attack_frame = 0
battle_wizard_attack_phase   = BATTLE_ATTACK_PHASE_APPROACH
battle_wizard_attack_frame   = 0
battle_fighter_attack_phase  = BATTLE_ATTACK_PHASE_APPROACH
battle_fighter_attack_frame  = 0
...
# マカダンス（味方バフ）演出ステート：踊り子が画面下に消える → バトルウィンドウがピンクに染まりダンスを再生
BATTLE_DANCE_PHASE_SINK  = 0  # 踊り子が画面下に消えるまで
...
# 攻防ステート内の行動順：女戦士の行動決定後に、生存している敵全体・踊り子・サムライ・女戦士を含めてランダムに決定する
# （-1 = 踊り子、-2 = サムライ、-3 = 女戦士、0以上 = 敵のインデックス。一巡したらコマンド選択ステートへ戻る）
battle_turn_order = []
battle_turn_index = 0

# 敵の攻撃演出ステート：接近 → ダメージ待機 → 白色点滅 → 後退（ムチ演出と同じ4ステートをそのまま流用する）
battle_enemy_attack_phase = BATTLE_ATTACK_PHASE_APPROACH
battle_enemy_attack_frame = 0
battle_attacking_enemy_index = -1  # 現在攻撃中の敵のインデックス（-1 = 攻撃側は踊り子／サムライ／女戦士）
battle_enemy_attack_target = -1    # 敵の攻撃対象（-1 = 踊り子、-2 = サムライ、-3 = 女戦士。敵の番開始時にランダム決定）

# ムチ：接近・後退中の残像履歴（直近 BATTLE_ATTACK_TRAIL_OFFSET_2 フレーム分の (x, 足元y, 画像高さ) を保持。古い順）
dancer_whip_trail = []
dancer_whip_trail_key = None  # 最後に記録したフレームの識別キー（(phase, frame)）。重複登録の防止用

# サムライの剣：接近・後退中の残像履歴（踊り子のムチと同じ仕組み）
samurai_attack_trail = []
samurai_attack_trail_key = None

# 女戦士の斧：接近・後退中の残像履歴（踊り子のムチと同じ仕組み）
warrior_attack_trail = []
warrior_attack_trail_key = None

# シスター・くノ一・魔法使い・武道家：接近・後退中の残像履歴
sister_attack_trail   = []
sister_attack_trail_key = None
kunoichi_attack_trail = []
kunoichi_attack_trail_key = None
wizard_attack_trail   = []
wizard_attack_trail_key = None
fighter_attack_trail  = []
fighter_attack_trail_key = None
...
# 心眼剣の斬撃エフェクト：水平1本。SHINGANKEN_SLASH_TOTAL_FRAMES 以上のとき非表示
battle_shingan_frame = -1  # 心眼剣攻撃全体の経過フレーム数（-1=非アクティブ）
battle_shingan_slash_frames = [SHINGANKEN_SLASH_TOTAL_FRAMES] * SHINGANKEN_SLASH_COUNT
...
# 音速剣の斬撃シーケンス（帰還後に発動）
battle_sonicken_slash_frame = -1  # 斬撃シーケンス全体の経過フレーム数（-1=非アクティブ）
battle_sonicken_slash_frames = [BATTLE_SLASH_TOTAL_FRAMES] * SONICKEN_SLASH_COUNT
battle_sonicken_slash_angles = [0.0] * SONICKEN_SLASH_COUNT
battle_sonicken_target_enemy_index = -1
...
DEBUG_FONT_SIZE       = 24  # FPS等のデバッグ表示・敵/踊り子/サムライのHPデバッグ表示のフォントサイズ
...
def _get_attack_target_m2p():
    """通常攻撃で最接近した時のメートル→ピクセル変換係数を返す。
    game_parameters.csv の BATTLE_ATTACK_TARGET_WINDOW_WIDTH_M から算出する。"""
    return SCREEN_W / float(_GAME_PARAMS.get('BATTLE_ATTACK_TARGET_WINDOW_WIDTH_M', 4.8))
...
def dancer_height_px():
    return int(DANCER_HEIGHT_M * METER_TO_PIXEL * zoom)
...
DANCER_FRONT_IMG_PATH = os.path.join(BASE_DIR, "..", "assets", "images", "characters", "bunny", "bunny_front.png")
...
VOICE_DANCER_DAMAGED_PATH = os.path.join(BASE_DIR, "..", "assets", "sound", "voices", "bunny_damaged_0.mp3")
...
def load_walk_images():
    ...
    for fname in files:
        ...
        character_art_top_height_m[fname] = scan_art_top_height_m(img, DANCER_HEIGHT_M)
        w, h = img.get_size()
        scale = dancer_height_px() / h
        img = pygame.transform.smoothscale(img, (int(w * scale), dancer_height_px()))
        images.append(img)
...
result_dancer_win_img  = None  # 勝利バストショット画像（bunny_front.pngをDANCER_HEIGHT_M分の高さに一度だけスケール）
...
dancer_front_img_raw = None  # 踊り子前姿画像（オリジナル） — ステータスウィンドウ用
...
voice_dancer_damaged = None  # 踊り子被弾やられボイス
...
#                      接近・攻撃演出ステートを初期化する（踊り子の番なら攻撃ボイスもここで再生する）
# ---------------------------------------------------------
def start_battle_turn():
    global battle_whip_phase, battle_whip_frame
    global battle_samurai_attack_phase, battle_samurai_attack_frame
    global battle_warrior_attack_phase, battle_warrior_attack_frame
    global battle_sister_attack_phase, battle_sister_attack_frame
    global battle_kunoichi_attack_phase, battle_kunoichi_attack_frame
    global battle_wizard_attack_phase, battle_wizard_attack_frame
    global battle_fighter_attack_phase, battle_fighter_attack_frame
    ...
    if attacker == -1:
        ...
        if battle_samurai_menu_selected_index == SAMURAI_MENU_INDEX_SHINGANKEN:
            ...
            # 刀・音速剣：どちらも近接攻撃（音速剣はとりあえず刀と同じ挙動）
            battle_samurai_attack_phase = BATTLE_ATTACK_PHASE_APPROACH
            battle_samurai_attack_frame = 0
            play_samurai_attack_voice()
    elif attacker == -3:
        # 女戦士の番：斧で攻撃する（流れ・ボイス再生のタイミングは踊り子のムチと同じ）
        battle_attacking_enemy_index = -1
        battle_warrior_attack_phase = BATTLE_ATTACK_PHASE_APPROACH
        battle_warrior_attack_frame = 0
...
def _apply_flame_attack(on_victory):
    ...
    if battle_flame_phase == BATTLE_FLAME_PHASE_CAST:
        ...
        battle_flame_frame += 1
        if battle_flame_frame >= BATTLE_ATTACK_FLASH_FRAMES:
            newly_defeated = []
...
def initialize():
    ...
    global result_dancer_win_img, result_samurai_win_img, result_warrior_win_img, enemy_img_raw, dance_images_raw, voice_win_by_number, voice_samurai_win_by_number, voice_warrior_win_by_number
    ...
    global dancer_front_img_raw, samurai_front_img_raw, samurai_back_img_raw, character_art_top_height_m
    ...
    global voice_battle_start_list, voice_samurai_battle_start_list, voice_warrior_battle_start_list, voice_samurai_attack_list, voice_warrior_attack_list, voice_attack_by_number, voice_goblin_damaged, voice_dancer_damaged, voice_samurai_damaged, voice_warrior_damaged, voice_dance, voice_katana_slash
    ...
    # ★ フィールド表示・ステータス前姿・勝利バストショットを兼ねる踊り子前姿画像（bunny_front.png）を読み込む
    # 歩行アニメーションは廃止し、フィールドでもこの画像を静止表示する
    dancer_front_img_raw = pygame.image.load(DANCER_FRONT_IMG_PATH).convert_alpha()
    walk_images = [dancer_front_img_raw]
    walk_image_filenames = ["bunny_front.png"]
    last_image = dancer_front_img_raw
    ...
    # （踊り子のdancer_front_img_rawは上で読み込み済み）
    ...
    character_art_top_height_m["bunny_front.png"] = scan_art_top_height_m(dancer_front_img_raw, DANCER_HEIGHT_M)
    character_art_top_height_m["bunny_back.png"]  = scan_art_top_height_m(battle_back_img_raw, DANCER_HEIGHT_M)
    ...
    # ★ 勝利バストショット画像（踊り子）：bunny_front.pngを、
    # RESULT_DANCER_WIN_WIDTH_M（ゲームウィンドウ幅が何mに相当するか）から算出した
    # DANCER_HEIGHT_M分の高さに一度だけスケールする（以後リザルト中の追加の拡縮は行わない）
    result_dancer_win_meter_to_pixel = SCREEN_W / RESULT_DANCER_WIN_WIDTH_M
    her_win_orig_w, her_win_orig_h = dancer_front_img_raw.get_size()
    her_win_img_h = max(1, int(DANCER_HEIGHT_M * result_dancer_win_meter_to_pixel))
    her_win_img_w = max(1, int(her_win_orig_w * her_win_img_h / her_win_orig_h))
    result_dancer_win_img = pygame.transform.smoothscale(dancer_front_img_raw, (her_win_img_w, her_win_img_h))
    ...
    voice_dancer_damaged = pygame.mixer.Sound(VOICE_DANCER_DAMAGED_PATH)
    voice_dancer_damaged.set_volume(VOICE_DANCER_DAMAGED_VOLUME)
...
# ---------------------------------------------------------
def enter_result_state(dancer_override=None,
    ...
    global result_flashout_dancer_override, result_flashout_samurai_override, result_flashout_is_samurai
    global result_flashout_warrior_override, result_flashout_is_warrior, result_flashout_active_char
    global result_flashout_killed_enemy_index
    ...
    global dancer_hp, samurai_hp, warrior_hp, sister_hp, kunoichi_hp, wizard_hp, fighter_hp
    global dancer_damage_anim_old_hp, dancer_damage_anim_new_hp, dancer_damage_anim_frame
    ...
    result_flashout_killed_enemy_index = killed_enemy_index
    ...
    result_text_delay_frame  = 0
    result_text_frame        = 0
    # フラッシュアウト中の踊り子／サムライ／女戦士表示位置・スケールの上書き指定（(x, 足元y, 画像高さ) のタプル、または None＝通常位置）
    # ムチ・剣・斧で最後の敵を倒した直後にリザルトへ遷移した場合などに、表示位置が瞬間的にスナップして見えるのを防ぐために使う
    result_flashout_dancer_override = dancer_override
    result_flashout_samurai_override = samurai_override
    result_flashout_warrior_override = warrior_override
    # フラッシュアウトで表示するキャラ：サムライ／女戦士以外がトドメを刺した場合も踊り子の代わりに表示する
    ...
    if warrior_finish:
        ...
        result_win_voice, result_victory_message, result_active_win_img = \
            _pick_voice_win(voice_win_by_number, RESULT_VICTORY_MESSAGES, result_dancer_win_img)
...
def _init_command_phase(phase):
    ...
    if phase == BATTLE_PHASE_COMMAND_DANCER:
        battle_menu_selected_index = 0
...
def battle_command_confirm():
    # 行動決定（Enterキー相当）：参戦キャラ全員の選択完了後、攻防ステートへ遷移する
    # battle_command_phase_orderの長さが参戦人数に応じて可変なため、動的なインデックスで制御する
    global battle_phase, battle_exchange_frame
    global battle_turn_order, battle_turn_index
    if battle_phase in battle_command_phase_order[:-1]:
    ...
    elif battle_phase == battle_command_phase_order[-1]:
        battle_phase = BATTLE_PHASE_EXCHANGE
        battle_exchange_frame = 0
        # 行動順：参戦キャラ全員と生存中の敵を含めてランダムにシャッフルする
        alive_indices = [i for i in range(len(enemy_defeated)) if not enemy_defeated[i]]
        battle_turn_order = list(battle_party) + alive_indices
...
def start_battle():
    ...
    global dancer_whip_trail_key, samurai_attack_trail_key, warrior_attack_trail_key
    global sister_attack_trail_key, kunoichi_attack_trail_key, wizard_attack_trail_key, fighter_attack_trail_key
    ...
    global dancer_macadance_used
    global battle_slash_frame, battle_slash_target_enemy_index
    global battle_shingan_frame, battle_shingan_slash_frames, battle_shingan_target_enemy_index
    global battle_sonicken_slash_frame, battle_sonicken_slash_frames, battle_sonicken_target_enemy_index
    ...
    dancer_whip_trail_key = None
    samurai_attack_trail.clear()
    samurai_attack_trail_key = None
    warrior_attack_trail.clear()
    warrior_attack_trail_key = None
    sister_attack_trail.clear()
    sister_attack_trail_key = None
    kunoichi_attack_trail.clear()
    kunoichi_attack_trail_key = None
    wizard_attack_trail.clear()
    wizard_attack_trail_key = None
    fighter_attack_trail.clear()
    fighter_attack_trail_key = None
    ...
    battle_sonicken_slash_frame = -1  # ★ 音速剣の斬撃エフェクトもリセット
    battle_sonicken_slash_frames = [BATTLE_SLASH_TOTAL_FRAMES] * SONICKEN_SLASH_COUNT
    battle_sonicken_target_enemy_index = -1
...
def process_ai_battle_input():
    ...
    all_command_phases = (BATTLE_PHASE_COMMAND_DANCER, BATTLE_PHASE_COMMAND_SAMURAI, BATTLE_PHASE_COMMAND_WARRIOR,
...
def process_input():
    ...
    for event in pygame.event.get():
        ...
        _all_cmd = (BATTLE_PHASE_COMMAND_DANCER, BATTLE_PHASE_COMMAND_SAMURAI, BATTLE_PHASE_COMMAND_WARRIOR,
                    BATTLE_PHASE_COMMAND_SISTER, BATTLE_PHASE_COMMAND_KUNOICHI,
                    BATTLE_PHASE_COMMAND_WIZARD, BATTLE_PHASE_COMMAND_FIGHTER)
        ...
        # Enterキー：踊り子の行動選択 → サムライの行動選択 → 攻防ステートへ遷移（コマンドウィンドウは非表示）／リザルト画面からフィールドへ
...
def update(dt):
    ...
    global dancer_damage_anim_old_hp, dancer_damage_anim_new_hp, dancer_damage_anim_frame, dancer_damage_anim_flash_color
    ...
    global battle_sonicken_slash_frame, battle_sonicken_slash_frames, battle_sonicken_slash_angles, battle_sonicken_target_enemy_index
    ...
    if game_state == STATE_RESULT:
        ...
        if battle_flashout_frame < BATTLE_FLASHOUT_FRAMES:
            ...
            # 対象敵描画つきフラッシュアウト中はその敵のダメージアニメを進める（HP点滅が白フェードの下で再生される）
            _ki = result_flashout_killed_enemy_index
            if 0 <= _ki < len(enemy_damage_anim_frame):
                if enemy_damage_anim_frame[_ki] < DAMAGE_ANIM_DONE_FRAME:
                    enemy_damage_anim_frame[_ki] += 1
            return
        # フェーズ②：白ウィンドウ静止
    ...
    if game_state == STATE_BATTLE:
        ...
        if dancer_damage_anim_frame < DAMAGE_ANIM_DONE_FRAME:
            dancer_damage_anim_frame += 1
        ...
        # ★ 心眼剣の斬撃エフェクト（水平1本）：刀の斬撃と同様、毎フレーム経過カウンタを進める
        for i in range(SHINGANKEN_SLASH_COUNT):
            if battle_shingan_slash_frames[i] < SHINGANKEN_SLASH_TOTAL_FRAMES:
                battle_shingan_slash_frames[i] += 1

        # ★ 音速剣の斬撃エフェクト：心眼剣と同様
        for i in range(SONICKEN_SLASH_COUNT):
            if battle_sonicken_slash_frames[i] < BATTLE_SLASH_TOTAL_FRAMES:
                battle_sonicken_slash_frames[i] += 1
        ...
        # ③ ズームアウト：静止が完了した瞬間にかけ声をランダムに選んで再生（注視先に応じて踊り子／サムライ／女戦士のかけ声を切り替える）
        ...
        if battle_phase == BATTLE_PHASE_EXCHANGE:
            if battle_menu_selected_index < len(_CHARA_MASTER['Dancer']['actions']):
                ...
                if battle_attacking_enemy_index == -1:
                    _attack_m2p = _get_attack_target_m2p()
                    _attack_ground_ratio = float(_GAME_PARAMS.get('BATTLE_ATTACK_TARGET_GROUND_Y_FROM_BOTTOM_RATIO', 0.1))
                    attacker = battle_turn_order[battle_turn_index]
                    if attacker == -2:
                        if battle_samurai_menu_selected_index == SAMURAI_MENU_INDEX_SHINGANKEN:
                            ...
                            for slash_i in range(SHINGANKEN_SLASH_COUNT):
                                ...
                                if battle_shingan_frame == slash_start_frame:
                                    ...
                                    battle_shingan_slash_angles[slash_i] = 0.0  # 水平（角度固定）
                            ...
                            if battle_shingan_frame == last_slash_start_frame + SHINGANKEN_SLASH_TOTAL_FRAMES:
                                # 斬撃アニメーション完了後：ダメージを適用し点滅演出を開始する
                                # （斬撃が全て終わってから HP 更新・フラッシュアウトに移ることで、演出の見切れを防ぐ）
                                ...
                                if enemy_hp[target] <= 0:
                                    ...
                                    # battle_annihilate は設定しない：フラッシュアウトが視覚的に覆うため

                                if all(enemy_defeated):
                                    # 最後の敵を撃破：フラッシュアウト中も対象敵を描画しつつリザルトへ遷移する
                            ...
                            if battle_samurai_menu_selected_index == SAMURAI_MENU_INDEX_SONICKEN:
                                # 音速剣：対象敵方向へ残像付きで接近（接近目標Y位置まで）→ ワープ（瞬間）→ 元位置へ残像付きで帰還
                                #         → 帰還後にランダム斬撃×SONICKEN_SLASH_COUNT → ダメージ適用 → ターン終了
                                if battle_samurai_attack_phase == BATTLE_ATTACK_PHASE_APPROACH:
                                    battle_samurai_attack_frame += 1
                                    if battle_samurai_attack_frame >= BATTLE_ATTACK_APPROACH_FRAMES:
                                        battle_samurai_attack_phase = BATTLE_ATTACK_PHASE_RETURN
                                        battle_samurai_attack_frame = 0
                                elif battle_samurai_attack_phase == BATTLE_ATTACK_PHASE_RETURN:
                                    battle_samurai_attack_frame += 1
                                    if battle_samurai_attack_frame >= BATTLE_ATTACK_APPROACH_FRAMES:
                                        # 帰還完了：斬撃シーケンスを開始する（FLASH フェーズをマーカーとして使用）
                                        battle_samurai_attack_phase = BATTLE_ATTACK_PHASE_FLASH
                                        battle_samurai_attack_frame = 0
                                        battle_sonicken_slash_frame = 0
                                        battle_sonicken_slash_frames[:] = [BATTLE_SLASH_TOTAL_FRAMES] * SONICKEN_SLASH_COUNT
                                        battle_sonicken_slash_angles[:] = [0.0] * SONICKEN_SLASH_COUNT
                                        battle_sonicken_target_enemy_index = battle_samurai_target_enemy_index
                                elif battle_samurai_attack_phase == BATTLE_ATTACK_PHASE_FLASH:
                                    # 音速剣：斬撃シーケンス（サムライは元位置で静止）
                                    battle_sonicken_slash_frame += 1
                                    for slash_i in range(SONICKEN_SLASH_COUNT):
                                        slash_start = slash_i * SONICKEN_SLASH_INTERVAL_FRAMES
                                        if battle_sonicken_slash_frame == slash_start:
                                            battle_sonicken_slash_frames[slash_i] = 0
                                            battle_sonicken_slash_angles[slash_i] = random.uniform(0.0, 2 * math.pi)
                                            if voice_katana_slash:
                                                voice_katana_slash.play()
                                    last_slash_start = (SONICKEN_SLASH_COUNT - 1) * SONICKEN_SLASH_INTERVAL_FRAMES
                                    if battle_sonicken_slash_frame == last_slash_start + BATTLE_SLASH_TOTAL_FRAMES:
                                        # 斬撃アニメーション完了後：ダメージを一括適用
                                        # （斬撃が全て終わってから HP 更新・フラッシュアウトに移ることで、演出の見切れを防ぐ）
                                        target = battle_sonicken_target_enemy_index
                                        old_display_hp = get_damage_display_hp(enemy_damage_anim_old_hp[target], enemy_damage_anim_new_hp[target], enemy_damage_anim_frame[target])
                                        enemy_hp[target] = max(0, enemy_hp[target] - SONICKEN_DAMAGE)
                                        enemy_damage_anim_old_hp[target] = old_display_hp
                                        enemy_damage_anim_new_hp[target] = enemy_hp[target]
                                        enemy_damage_anim_frame[target]  = 0
                                        enemy_damage_anim_flash_color[target] = DAMAGE_FLASH_COLOR_SWORD
                                        if enemy_hp[target] <= 0:
                                            ...
                                            # battle_annihilate は設定しない：フラッシュアウトが視覚的に覆うため
                                        if all(enemy_defeated):
                                            son_x, son_bottom_y, son_img_h = get_samurai_base_position()
                                            enter_result_state(samurai_finish=True,
                                                               samurai_override=(son_x, son_bottom_y, son_img_h),
                                                               killed_enemy_index=target)
                                    if (not all(enemy_defeated)
                                            and battle_sonicken_slash_frame >= last_slash_start + BATTLE_SLASH_TOTAL_FRAMES):
                                        battle_samurai_target_enemy_index = find_alive_enemy_index(battle_samurai_target_enemy_index, 1)
                                        advance_battle_turn()
                            else:
                                # サムライの番：刀で攻撃対象（battle_samurai_target_enemy_index）を攻撃する
                                # （流れはムチと同じ４ステート。攻撃ボイスは再生しない）
                                if battle_samurai_attack_phase == BATTLE_ATTACK_PHASE_APPROACH:
                                    battle_samurai_attack_frame += 1
                                    if battle_samurai_attack_frame >= BATTLE_ATTACK_APPROACH_FRAMES:
                                        battle_samurai_attack_phase = BATTLE_ATTACK_PHASE_DAMAGE_WAIT
                                        battle_samurai_attack_frame = 0
                                        ...
                                        if voice_katana_slash:
                                            voice_katana_slash.play()
                                elif battle_samurai_attack_phase == BATTLE_ATTACK_PHASE_DAMAGE_WAIT:
                                    battle_samurai_attack_frame += 1
                                    if battle_samurai_attack_frame >= BATTLE_ATTACK_DAMAGE_DELAY_FRAMES:
                                        battle_samurai_attack_phase = BATTLE_ATTACK_PHASE_FLASH
                                        battle_samurai_attack_frame = 0
                                        if voice_goblin_damaged:
                                            voice_goblin_damaged.play()
                                    ...
                                    if battle_samurai_attack_frame >= BATTLE_ATTACK_FLASH_FRAMES:
                                        ...
                                        if all(enemy_defeated):
                                            ...
                                            # （踊り子のムチと同様、最接近位置をそのままリザルトのフラッシュアウトに引き継ぐ）
                                            # サムライがトドメを刺したため、リザルトはサムライのバストショット・ボイス・メッセージを表示する
                                            sword_target_x = int(SCREEN_W * ENEMY_X_RATIOS[target])
                                            sword_target_bottom_y = SCREEN_H - int(SCREEN_H * _attack_ground_ratio)
                                            sword_target_img_h = int(SAMURAI_HEIGHT_M * _attack_m2p)
                                            enter_result_state(samurai_finish=True,
                        ...
                        # （流れは踊り子のムチと同じ4ステート）
                        if battle_warrior_attack_phase == BATTLE_ATTACK_PHASE_APPROACH:
                            battle_warrior_attack_frame += 1
                            if battle_warrior_attack_frame >= BATTLE_ATTACK_APPROACH_FRAMES:
                                battle_warrior_attack_phase = BATTLE_ATTACK_PHASE_DAMAGE_WAIT
                                battle_warrior_attack_frame = 0
                        elif battle_warrior_attack_phase == BATTLE_ATTACK_PHASE_DAMAGE_WAIT:
                            battle_warrior_attack_frame += 1
                            if battle_warrior_attack_frame >= BATTLE_ATTACK_DAMAGE_DELAY_FRAMES:
                                battle_warrior_attack_phase = BATTLE_ATTACK_PHASE_FLASH
                                battle_warrior_attack_frame = 0
                                if voice_goblin_damaged:
                                    voice_goblin_damaged.play()
                        elif battle_warrior_attack_phase == BATTLE_ATTACK_PHASE_FLASH:
                            battle_warrior_attack_frame += 1
                            if battle_warrior_attack_frame >= BATTLE_ATTACK_FLASH_FRAMES:
                                # ダメージ演出完了の瞬間にHPを更新する（0未満にはならない。0になっていたら撃破扱い）
                                ...
                                if all(enemy_defeated):
                                    ...
                                    axe_target_bottom_y = SCREEN_H - int(SCREEN_H * _attack_ground_ratio)
                                    axe_target_img_h = int(WARRIOR_HEIGHT_M * _attack_m2p)
                                    enter_result_state(warrior_finish=True,
                                                        warrior_override=(axe_target_x, axe_target_bottom_y, axe_target_img_h),
                                                        killed_enemy_index=target)
                                else:
                                    battle_warrior_attack_phase = BATTLE_ATTACK_PHASE_RETURN
                                    battle_warrior_attack_frame = 0
                        elif battle_warrior_attack_phase == BATTLE_ATTACK_PHASE_RETURN:
                            battle_warrior_attack_frame += 1
                            if battle_warrior_attack_frame >= BATTLE_ATTACK_APPROACH_FRAMES:
                                battle_warrior_target_enemy_index = find_alive_enemy_index(battle_warrior_target_enemy_index, 1)
                                advance_battle_turn()
                    elif attacker == -4:
                        if battle_sister_attack_phase == BATTLE_ATTACK_PHASE_APPROACH:
                            battle_sister_attack_frame += 1
                            if battle_sister_attack_frame >= BATTLE_ATTACK_APPROACH_FRAMES:
                                battle_sister_attack_phase = BATTLE_ATTACK_PHASE_DAMAGE_WAIT
                                battle_sister_attack_frame = 0
                        elif battle_sister_attack_phase == BATTLE_ATTACK_PHASE_DAMAGE_WAIT:
                            battle_sister_attack_frame += 1
                            if battle_sister_attack_frame >= BATTLE_ATTACK_DAMAGE_DELAY_FRAMES:
                                battle_sister_attack_phase = BATTLE_ATTACK_PHASE_FLASH
                                battle_sister_attack_frame = 0
                                if voice_goblin_damaged:
                                    voice_goblin_damaged.play()
                        elif battle_sister_attack_phase == BATTLE_ATTACK_PHASE_FLASH:
                            battle_sister_attack_frame += 1
                            if battle_sister_attack_frame >= BATTLE_ATTACK_FLASH_FRAMES:
                                ...
                                if all(enemy_defeated):
                                    ...
                                    sis_target_bottom_y = SCREEN_H - int(SCREEN_H * _attack_ground_ratio)
                                    sis_target_img_h = int(SISTER_HEIGHT_M * _attack_m2p)
                                    enter_result_state(sister_finish=True,
                                                        sister_override=(sis_target_x, sis_target_bottom_y, sis_target_img_h),
                                                        killed_enemy_index=target)
                                else:
                                    battle_sister_attack_phase = BATTLE_ATTACK_PHASE_RETURN
                                    battle_sister_attack_frame = 0
                        elif battle_sister_attack_phase == BATTLE_ATTACK_PHASE_RETURN:
                            battle_sister_attack_frame += 1
                            if battle_sister_attack_frame >= BATTLE_ATTACK_APPROACH_FRAMES:
                                battle_sister_target_enemy_index = find_alive_enemy_index(battle_sister_target_enemy_index, 1)
                                advance_battle_turn()
                    elif attacker == -5:
                        if battle_kunoichi_attack_phase == BATTLE_ATTACK_PHASE_APPROACH:
                            battle_kunoichi_attack_frame += 1
                            if battle_kunoichi_attack_frame >= BATTLE_ATTACK_APPROACH_FRAMES:
                                battle_kunoichi_attack_phase = BATTLE_ATTACK_PHASE_DAMAGE_WAIT
                                battle_kunoichi_attack_frame = 0
                        elif battle_kunoichi_attack_phase == BATTLE_ATTACK_PHASE_DAMAGE_WAIT:
                            battle_kunoichi_attack_frame += 1
                            if battle_kunoichi_attack_frame >= BATTLE_ATTACK_DAMAGE_DELAY_FRAMES:
                                battle_kunoichi_attack_phase = BATTLE_ATTACK_PHASE_FLASH
                                battle_kunoichi_attack_frame = 0
                                if voice_goblin_damaged:
                                    voice_goblin_damaged.play()
                        elif battle_kunoichi_attack_phase == BATTLE_ATTACK_PHASE_FLASH:
                            battle_kunoichi_attack_frame += 1
                            if battle_kunoichi_attack_frame >= BATTLE_ATTACK_FLASH_FRAMES:
                                ...
                                if all(enemy_defeated):
                                    ...
                                    kun_target_bottom_y = SCREEN_H - int(SCREEN_H * _attack_ground_ratio)
                                    kun_target_img_h = int(KUNOICHI_HEIGHT_M * _attack_m2p)
                                    enter_result_state(kunoichi_finish=True,
                                                        kunoichi_override=(kun_target_x, kun_target_bottom_y, kun_target_img_h),
                                                        killed_enemy_index=target)
                                else:
                                    battle_kunoichi_attack_phase = BATTLE_ATTACK_PHASE_RETURN
                                    battle_kunoichi_attack_frame = 0
                        elif battle_kunoichi_attack_phase == BATTLE_ATTACK_PHASE_RETURN:
                            battle_kunoichi_attack_frame += 1
                            if battle_kunoichi_attack_frame >= BATTLE_ATTACK_APPROACH_FRAMES:
                                battle_kunoichi_target_enemy_index = find_alive_enemy_index(battle_kunoichi_target_enemy_index, 1)
                                advance_battle_turn()
                        ...
                        if _get_wizard_action() == '火炎放射':
                            ...
                            if battle_wizard_attack_phase == BATTLE_ATTACK_PHASE_APPROACH:
                                battle_wizard_attack_frame += 1
                                if battle_wizard_attack_frame >= BATTLE_ATTACK_APPROACH_FRAMES:
                                    battle_wizard_attack_phase = BATTLE_ATTACK_PHASE_DAMAGE_WAIT
                                    battle_wizard_attack_frame = 0
                            elif battle_wizard_attack_phase == BATTLE_ATTACK_PHASE_DAMAGE_WAIT:
                                battle_wizard_attack_frame += 1
                                if battle_wizard_attack_frame >= BATTLE_ATTACK_DAMAGE_DELAY_FRAMES:
                                    battle_wizard_attack_phase = BATTLE_ATTACK_PHASE_FLASH
                                    battle_wizard_attack_frame = 0
                                    if voice_goblin_damaged:
                                        voice_goblin_damaged.play()
                            elif battle_wizard_attack_phase == BATTLE_ATTACK_PHASE_FLASH:
                                battle_wizard_attack_frame += 1
                                if battle_wizard_attack_frame >= BATTLE_ATTACK_FLASH_FRAMES:
                                    ...
                                    if all(enemy_defeated):
                                        ...
                                        wiz_target_bottom_y = SCREEN_H - int(SCREEN_H * _attack_ground_ratio)
                                        wiz_target_img_h = int(WIZARD_HEIGHT_M * _attack_m2p)
                                        enter_result_state(wizard_finish=True,
                                                            wizard_override=(wiz_target_x, wiz_target_bottom_y, wiz_target_img_h),
                                                            killed_enemy_index=target)
                                    else:
                                        battle_wizard_attack_phase = BATTLE_ATTACK_PHASE_RETURN
                                        battle_wizard_attack_frame = 0
                            elif battle_wizard_attack_phase == BATTLE_ATTACK_PHASE_RETURN:
                                battle_wizard_attack_frame += 1
                                if battle_wizard_attack_frame >= BATTLE_ATTACK_APPROACH_FRAMES:
                                    battle_wizard_target_enemy_index = find_alive_enemy_index(battle_wizard_target_enemy_index, 1)
                                    advance_battle_turn()
                    elif attacker == -7:
                        if battle_fighter_attack_phase == BATTLE_ATTACK_PHASE_APPROACH:
                            battle_fighter_attack_frame += 1
                            if battle_fighter_attack_frame >= BATTLE_ATTACK_APPROACH_FRAMES:
                                battle_fighter_attack_phase = BATTLE_ATTACK_PHASE_DAMAGE_WAIT
                                battle_fighter_attack_frame = 0
                        elif battle_fighter_attack_phase == BATTLE_ATTACK_PHASE_DAMAGE_WAIT:
                            battle_fighter_attack_frame += 1
                            if battle_fighter_attack_frame >= BATTLE_ATTACK_DAMAGE_DELAY_FRAMES:
                                battle_fighter_attack_phase = BATTLE_ATTACK_PHASE_FLASH
                                battle_fighter_attack_frame = 0
                                if voice_goblin_damaged:
                                    voice_goblin_damaged.play()
                        elif battle_fighter_attack_phase == BATTLE_ATTACK_PHASE_FLASH:
                            battle_fighter_attack_frame += 1
                            if battle_fighter_attack_frame >= BATTLE_ATTACK_FLASH_FRAMES:
                                ...
                                if all(enemy_defeated):
                                    ...
                                    fig_target_bottom_y = SCREEN_H - int(SCREEN_H * _attack_ground_ratio)
                                    fig_target_img_h = int(FIGHTER_HEIGHT_M * _attack_m2p)
                                    enter_result_state(fighter_finish=True,
                                                        fighter_override=(fig_target_x, fig_target_bottom_y, fig_target_img_h),
                                                        killed_enemy_index=target)
                                else:
                                    battle_fighter_attack_phase = BATTLE_ATTACK_PHASE_RETURN
                                    battle_fighter_attack_frame = 0
                        elif battle_fighter_attack_phase == BATTLE_ATTACK_PHASE_RETURN:
                            battle_fighter_attack_frame += 1
                            if battle_fighter_attack_frame >= BATTLE_ATTACK_APPROACH_FRAMES:
                                battle_fighter_target_enemy_index = find_alive_enemy_index(battle_fighter_target_enemy_index, 1)
                                advance_battle_turn()
                        ...
                        if battle_whip_phase == BATTLE_ATTACK_PHASE_APPROACH:
                            battle_whip_frame += 1
                            if battle_whip_frame >= BATTLE_ATTACK_APPROACH_FRAMES:
                                battle_whip_phase = BATTLE_ATTACK_PHASE_DAMAGE_WAIT
                                battle_whip_frame = 0
                        elif battle_whip_phase == BATTLE_ATTACK_PHASE_DAMAGE_WAIT:
                            battle_whip_frame += 1
                            if battle_whip_frame >= BATTLE_ATTACK_DAMAGE_DELAY_FRAMES:
                                battle_whip_phase = BATTLE_ATTACK_PHASE_FLASH
                                battle_whip_frame = 0
                                if voice_goblin_damaged:
                                    voice_goblin_damaged.play()
                        elif battle_whip_phase == BATTLE_ATTACK_PHASE_FLASH:
                            battle_whip_frame += 1
                            if battle_whip_frame >= BATTLE_ATTACK_FLASH_FRAMES:
                                # ダメージ演出完了の瞬間にHPを更新する（0未満にはならない。0になっていたら撃破扱い）
                                ...
                                if all(enemy_defeated):
                                    ...
                                    whip_target_bottom_y = SCREEN_H - int(SCREEN_H * _attack_ground_ratio)
                                    whip_target_img_h = int(DANCER_HEIGHT_M * _attack_m2p)
                                    enter_result_state(dancer_override=(whip_target_x, whip_target_bottom_y, whip_target_img_h),
                                                    killed_enemy_index=target)
                                else:
                                    # 敵が残っている場合は今まで通り元の位置へ後退する
                                    battle_whip_phase = BATTLE_ATTACK_PHASE_RETURN
                                    battle_whip_frame = 0
                        elif battle_whip_phase == BATTLE_ATTACK_PHASE_RETURN:
                            battle_whip_frame += 1
                            if battle_whip_frame >= BATTLE_ATTACK_APPROACH_FRAMES:
                    ...
                    # 敵の攻撃ボイスは未実装のため再生しない
                    if battle_enemy_attack_phase == BATTLE_ATTACK_PHASE_APPROACH:
                        battle_enemy_attack_frame += 1
                        if battle_enemy_attack_frame >= BATTLE_ATTACK_APPROACH_FRAMES:
                            battle_enemy_attack_phase = BATTLE_ATTACK_PHASE_DAMAGE_WAIT
                            battle_enemy_attack_frame = 0
                    elif battle_enemy_attack_phase == BATTLE_ATTACK_PHASE_DAMAGE_WAIT:
                        battle_enemy_attack_frame += 1
                        if battle_enemy_attack_frame >= BATTLE_ATTACK_DAMAGE_DELAY_FRAMES:
                            battle_enemy_attack_phase = BATTLE_ATTACK_PHASE_FLASH
                            battle_enemy_attack_frame = 0
                    elif battle_enemy_attack_phase == BATTLE_ATTACK_PHASE_FLASH:
                        battle_enemy_attack_frame += 1
                        if battle_enemy_attack_frame >= BATTLE_ATTACK_FLASH_FRAMES:
                            # ダメージ演出完了の瞬間に攻撃対象（踊り子またはサムライ）のHPを更新する（0未満にはならない。0になっても現状は何も起きない）
                            ...
                            if battle_enemy_attack_target == -2:
                                ...
                                old_display_hp = get_damage_display_hp(dancer_damage_anim_old_hp, dancer_damage_anim_new_hp, dancer_damage_anim_frame)
                                dancer_hp = max(0, dancer_hp - damage)
                                dancer_damage_anim_old_hp = old_display_hp
                                dancer_damage_anim_new_hp = dancer_hp
                                dancer_damage_anim_frame  = 0
                                dancer_damage_anim_flash_color = DAMAGE_FLASH_COLOR_ENEMY_ATTACK
                                if voice_dancer_damaged:
                                    voice_dancer_damaged.play()
                            battle_enemy_attack_phase = BATTLE_ATTACK_PHASE_RETURN
                            battle_enemy_attack_frame = 0
                    elif battle_enemy_attack_phase == BATTLE_ATTACK_PHASE_RETURN:
                        battle_enemy_attack_frame += 1
                        if battle_enemy_attack_frame >= BATTLE_ATTACK_APPROACH_FRAMES:
                            # 元の位置に戻り終えたら次の番へ進める（踊り子が力尽きることはないため、そのまま進行する）
                            advance_battle_turn()
            else:
...
def render_field():
    ...
    target_h = dancer_height_px()
    orig_w, orig_h = dancer_front_img_raw.get_size()
    scaled_img = pygame.transform.smoothscale(dancer_front_img_raw, (max(1, int(orig_w * target_h / orig_h)), target_h))
    display_image = apply_hp_grayscale(scaled_img, "bunny_front.png", DANCER_HEIGHT_M, dancer_hp, DANCER_MAX_HP)
    ...
    if is_debug:
        ...
        draw_art_top_debug_line("bunny_front.png", DANCER_HEIGHT_M, sy, display_image.get_height(), rect.left, rect.right)  # ★ [デバッグ] 実際の絵の上端
...
# ---------------------------------------------------------
# blit_dancer_trail_image(raw_img, pos, alpha, clip_rect)：残像（直近フレームの位置・スケールの踊り子画像）を半透明で描画する
# pos は (x, 足元y, 画像高さ) のタプル。alpha は不透明度（0.0～1.0）
# ---------------------------------------------------------
def blit_dancer_trail_image(raw_img, pos, alpha, clip_rect):
    x, boty, h = pos
    orig_w, orig_h = raw_img.get_size()
...
# 点p1（軌跡の終端）から点p2（軌跡の先端）へ向かう方向を対称軸とした、水色の細い二等辺三角形の軌跡を描画する。
# 先端側（seg_end）を頂点（先細り）、終端側（seg_start）を底辺（幅広）とする。
# frameに応じて「描画(0～DRAW_FRAMES-1)」→「維持」→「消去」の3段階で軌跡の範囲が変化する
# ---------------------------------------------------------
def draw_slash_triangle(p1, p2, frame, hold_frames=BATTLE_SLASH_HOLD_FRAMES):
    ...
    elif frame < BATTLE_SLASH_DRAW_FRAMES + hold_frames:
        seg_start, seg_end = p1, p2
    else:
        erase_frame = frame - BATTLE_SLASH_DRAW_FRAMES - hold_frames
        t = (erase_frame + 1) / BATTLE_SLASH_ERASE_FRAMES if BATTLE_SLASH_ERASE_FRAMES > 0 else 1.0
        seg_start, seg_end = lerp_point(p1, p2, t), p2

    # 頂点(apex)＝seg_end（先端）、底辺の中心＝seg_start（終端）とし、底辺はseg_start→seg_end方向に対して
    ...
    points = [
        seg_end,
        (seg_start[0] + nx * half_base, seg_start[1] + ny * half_base),
        (seg_start[0] - nx * half_base, seg_start[1] - ny * half_base),
    ]
...
# 音速剣の斬撃エフェクト：刀の軌跡（矩形の右上から左下）と同じ中心・長さの線分を、
...
# ---------------------------------------------------------
# draw_shinganken_slash(rect, top_y, frame)：
# 心眼剣の斬撃エフェクト：敵の上端Y（top_y）と下端Y（rect.bottom）の中央を Y 座標として、
# 画面左端（終端・幅広）から画面右端（先端・細）への水平な剣閃を SHINGANKEN_SLASH_HOLD_FRAMES で描画する
# ---------------------------------------------------------
def draw_shinganken_slash(rect, top_y, frame):
    center_y = (top_y + rect.bottom) / 2.0
    p1 = (0, center_y)         # 左端（軌跡の終端・底辺側）
    p2 = (SCREEN_W, center_y)  # 右端（軌跡の先端・頂点側）
    draw_slash_triangle(p1, p2, frame, hold_frames=SHINGANKEN_SLASH_HOLD_FRAMES)
...
def render_battle():
    ...
    # ★ バトルメインウィンドウが開ききった時点での下端（踊り子の足元位置の基準として使う。開くアニメ中も移動させない）
    band_bottom_full = SCREEN_H // 2 - BATTLE_MAIN_WINDOW_HEIGHT // 2 + BATTLE_MAIN_WINDOW_HEIGHT

    # ★ 踊り子・サムライ・女戦士の基準位置（攻撃演出等による移動を含まない通常時の位置）を先に計算する
    # （敵の攻撃接近先の算出に、各キャラの描画より先に必要となるため）
    # ズームアウトのカメラは、battle_focus_character に応じて踊り子・サムライ・女戦士のいずれかの足元に注視する。
    # 他のキャラは、ワールド座標系での相対位置（踊り子を基準に、サムライは1m右・女戦士は2m右）を保ったまま表示する
    ...
    dancer_base_bottom_y = focus_bottom_y
    ...
    dancer_base_x  = SCREEN_W // 2 + int(battle_character_world_offset_m[-1] * focus_meter_to_pixel)
    ...
    # ★ 敵（goblin）の描画：黒シルエット → 通常表示への解除（踊り子の奥＝先に描画）
    if enemy_img_raw:
        enemy_orig_w, enemy_orig_h = enemy_img_raw.get_size()

        # ズームアウト進行度（0.0 → 1.0）：踊り子と同じタイミングで指定スケールへ収束
        ...
        if enemy_attack_active:
            if battle_enemy_attack_phase == BATTLE_ATTACK_PHASE_APPROACH:
                raw_t = (battle_enemy_attack_frame / BATTLE_ATTACK_APPROACH_FRAMES) if BATTLE_ATTACK_APPROACH_FRAMES > 0 else 1.0
                enemy_attack_t = 1.0 - (1.0 - raw_t) ** 2          # 踊り子に近づくほど減速
            elif battle_enemy_attack_phase in (BATTLE_ATTACK_PHASE_DAMAGE_WAIT, BATTLE_ATTACK_PHASE_FLASH):
                enemy_attack_t = 1.0
            else:  # BATTLE_ATTACK_PHASE_RETURN
                raw_t = (battle_enemy_attack_frame / BATTLE_ATTACK_APPROACH_FRAMES) if BATTLE_ATTACK_APPROACH_FRAMES > 0 else 1.0
        ...
        sonicken_target_top_y = None  # 音速剣の斬撃エフェクト用
        screen.set_clip(clip_rect)
        for i, x_ratio in enumerate(ENEMY_X_RATIOS):
            ...
            if is_attacking:
                # ★ 敵の攻撃：攻撃対象（踊り子またはサムライ）に接近 → 指定スケール・位置で最接近 → 元の位置へ後退
                ...
                if battle_enemy_attack_target == -2:
                    ...
                    attack_target_img_h = int(SCREEN_H * BATTLE_ENEMY_ATTACK_TARGET_SCALE * (SAMURAI_HEIGHT_M / DANCER_HEIGHT_M))
                elif battle_enemy_attack_target == -3:
                    # 女戦士への攻撃：女戦士の基準位置に向けて接近する
                    attack_target_x = warrior_base_x
                    attack_target_bottom_y = band_bottom - int(SCREEN_H * BATTLE_ENEMY_ATTACK_TARGET_GROUND_Y_OFFSET_RATIO)
                    attack_target_img_h = int(SCREEN_H * BATTLE_ENEMY_ATTACK_TARGET_SCALE * (WARRIOR_HEIGHT_M / DANCER_HEIGHT_M))
                elif battle_enemy_attack_target == -4:
                    attack_target_x = sister_base_x
                    attack_target_bottom_y = band_bottom - int(SCREEN_H * BATTLE_ENEMY_ATTACK_TARGET_GROUND_Y_OFFSET_RATIO)
                    attack_target_img_h = int(SCREEN_H * BATTLE_ENEMY_ATTACK_TARGET_SCALE * (SISTER_HEIGHT_M / DANCER_HEIGHT_M))
                elif battle_enemy_attack_target == -5:
                    attack_target_x = kunoichi_base_x
                    attack_target_bottom_y = band_bottom - int(SCREEN_H * BATTLE_ENEMY_ATTACK_TARGET_GROUND_Y_OFFSET_RATIO)
                    attack_target_img_h = int(SCREEN_H * BATTLE_ENEMY_ATTACK_TARGET_SCALE * (KUNOICHI_HEIGHT_M / DANCER_HEIGHT_M))
                elif battle_enemy_attack_target == -6:
                    attack_target_x = wizard_base_x
                    attack_target_bottom_y = band_bottom - int(SCREEN_H * BATTLE_ENEMY_ATTACK_TARGET_GROUND_Y_OFFSET_RATIO)
                    attack_target_img_h = int(SCREEN_H * BATTLE_ENEMY_ATTACK_TARGET_SCALE * (WIZARD_HEIGHT_M / DANCER_HEIGHT_M))
                elif battle_enemy_attack_target == -7:
                    attack_target_x = fighter_base_x
                    attack_target_bottom_y = band_bottom - int(SCREEN_H * BATTLE_ENEMY_ATTACK_TARGET_GROUND_Y_OFFSET_RATIO)
                    attack_target_img_h = int(SCREEN_H * BATTLE_ENEMY_ATTACK_TARGET_SCALE * (FIGHTER_HEIGHT_M / DANCER_HEIGHT_M))
                else:
                    # 踊り子への攻撃：踊り子の基準位置に向けて接近する
                    attack_target_x = dancer_base_x
                    attack_target_bottom_y = band_bottom - int(SCREEN_H * BATTLE_ENEMY_ATTACK_TARGET_GROUND_Y_OFFSET_RATIO)
            ...
            if i == battle_shingan_target_enemy_index:
                for slash_i in range(SHINGANKEN_SLASH_COUNT):
                    if battle_shingan_slash_frames[slash_i] < SHINGANKEN_SLASH_TOTAL_FRAMES:
                        shingan_target_top_y = get_art_top_y("goblin_idle.png", ENEMY_GOBLIN_HEIGHT_M, enemy_img_bottom_y, final_img_h)
                        break

            # ★ 音速剣の斬撃エフェクト：同様に上端Y座標を記憶する
            if i == battle_sonicken_target_enemy_index:
                for slash_i in range(SONICKEN_SLASH_COUNT):
                    if battle_sonicken_slash_frames[slash_i] < BATTLE_SLASH_TOTAL_FRAMES:
                        sonicken_target_top_y = get_art_top_y("goblin_idle.png", ENEMY_GOBLIN_HEIGHT_M, enemy_img_bottom_y, final_img_h)
                        break
    ...
    # ★ 踊り子後ろ姿の描画（マカダンスのダンス中は画面下に消えているため描画しないが、
    #    サムライはこのブロック内で描画されるため、ダンス中もブロック自体は実行する）
    if battle_back_img_raw:
        orig_w, orig_h = battle_back_img_raw.get_size()

        # 画像高さ・足元位置：上で計算済みの踊り子の基準位置をそのまま使う
        img_h = dancer_base_img_h
        bottom_y = dancer_base_bottom_y
        ...
        if whip_active:
            if battle_whip_phase == BATTLE_ATTACK_PHASE_APPROACH:
                raw_t = (battle_whip_frame / BATTLE_ATTACK_APPROACH_FRAMES) if BATTLE_ATTACK_APPROACH_FRAMES > 0 else 1.0
                whip_t = 1.0 - (1.0 - raw_t) ** 2          # 敵に近づくほど減速
            elif battle_whip_phase in (BATTLE_ATTACK_PHASE_DAMAGE_WAIT, BATTLE_ATTACK_PHASE_FLASH):
                whip_t = 1.0
            else:  # BATTLE_ATTACK_PHASE_RETURN
                raw_t = (battle_whip_frame / BATTLE_ATTACK_APPROACH_FRAMES) if BATTLE_ATTACK_APPROACH_FRAMES > 0 else 1.0
            ...
            # 最接近時の踊り子の足元位置：横方向は敵に合わせるが、縦方向（接地ライン）は敵とは別の比率で指定する
            whip_target_bottom_y = SCREEN_H - int(SCREEN_H * _attack_ground_ratio)
            whip_target_img_h = int(DANCER_HEIGHT_M * _attack_m2p)

            dancer_x = int(dancer_x + (whip_target_x - dancer_x) * whip_t)
        ...
        # ★ 現在HPに応じて、画像下部（絵の範囲のうち減った体力分）をグレースケール化する
        # ★ ダメージ表現アニメーション：表示HPは実HPに即時追従せず、old_hp→new_hpへ数フレームかけて変化させる
        dancer_display_hp = get_damage_display_hp(dancer_damage_anim_old_hp, dancer_damage_anim_new_hp, dancer_damage_anim_frame)
        dancer_display_img_raw = apply_hp_grayscale(battle_back_img_raw, "bunny_back.png", DANCER_HEIGHT_M, int(round(dancer_display_hp)), DANCER_MAX_HP)
        dancer_display_img_raw = apply_damage_flash(dancer_display_img_raw, "bunny_back.png", DANCER_HEIGHT_M, dancer_damage_anim_old_hp, dancer_damage_anim_new_hp, DANCER_MAX_HP, dancer_damage_anim_frame, dancer_damage_anim_flash_color)
        img, img_pos, img_rect = smoothscale_visible(dancer_display_img_raw, final_img_w, final_img_h, (dancer_x, bottom_y), clip_rect)

        # ★ 踊り子の描画本体（サムライとの前後関係を切り替えられるよう、関数化して呼び出しタイミングを後で決める）
        def draw_dancer_sprite():
            ...
            if is_debug:
                ...
                draw_art_top_debug_line("bunny_back.png", DANCER_HEIGHT_M, bottom_y, final_img_h, img_rect.left, img_rect.right)  # ★ [デバッグ] 実際の絵の上端
            screen.set_clip(None)

            # [デバッグ表示] 踊り子のHPを「現在HP/最大HP」の文字列で頭上に表示する
            dancer_hp_text = font.render(f"{dancer_hp}/{DANCER_MAX_HP}", True, (255, 255, 255))
            dancer_hp_rect = dancer_hp_text.get_rect(midbottom=(img_rect.centerx, img_rect.top - 4))
            screen.blit(dancer_hp_text, dancer_hp_rect)

        # ★ サムライ（仲間キャラ）の後ろ姿：踊り子の1m右を基準位置とし、剣（近接攻撃）の番には敵に接近・後退する
        sam_back_img_raw = samurai_back_img_raw
        sam_back_art_key = "samurai_back.png"
        if sam_back_img_raw:
            sam_orig_w, sam_orig_h = sam_back_img_raw.get_size()

            # ★ 剣（近接攻撃）：敵に接近 → 最接近で停止 → 元の位置へ後退（流れは踊り子のムチと同じ）
            ...
            # 刀：心眼剣選択時は接近せず、その場にとどまる。音速剣は別処理
            sword_active    = samurai_active and battle_samurai_menu_selected_index == SAMURAI_MENU_INDEX_SWORD
            sonicken_active = samurai_active and battle_samurai_menu_selected_index == SAMURAI_MENU_INDEX_SONICKEN
            sword_target_x = int(SCREEN_W * ENEMY_X_RATIOS[battle_samurai_target_enemy_index])
            sword_target_bottom_y = SCREEN_H - int(SCREEN_H * _attack_ground_ratio)
            sword_target_img_h = int(SAMURAI_HEIGHT_M * _attack_m2p)
            if sword_active:
                if battle_samurai_attack_phase == BATTLE_ATTACK_PHASE_APPROACH:
                    raw_t = (battle_samurai_attack_frame / BATTLE_ATTACK_APPROACH_FRAMES) if BATTLE_ATTACK_APPROACH_FRAMES > 0 else 1.0
                    sword_t = 1.0 - (1.0 - raw_t) ** 2
                elif battle_samurai_attack_phase in (BATTLE_ATTACK_PHASE_DAMAGE_WAIT, BATTLE_ATTACK_PHASE_FLASH):
                    sword_t = 1.0
                else:  # RETURN
                    raw_t = (battle_samurai_attack_frame / BATTLE_ATTACK_APPROACH_FRAMES) if BATTLE_ATTACK_APPROACH_FRAMES > 0 else 1.0
                    sword_t = (1.0 - raw_t) ** 2
            ...
            elif sonicken_active:
                # 音速剣：サムライ足元→対象敵足元のベクトル方向へ移動
                dir_x = sword_target_x - sam_base_x
                dir_y = sword_target_bottom_y - sam_base_bottom_y
                sonicken_approach_bottom_y = SCREEN_H - int(SCREEN_H * SONICKEN_APPROACH_Y_FROM_BOTTOM_RATIO)
                sonicken_warp_bottom_y     = SCREEN_H - int(SCREEN_H * SONICKEN_WARP_Y_FROM_BOTTOM_RATIO)
                if dir_y != 0:
                    t_app  = (sonicken_approach_bottom_y - sam_base_bottom_y) / dir_y
                    t_warp = (sonicken_warp_bottom_y     - sam_base_bottom_y) / dir_y
                else:
                    t_app = t_warp = 0.0
                sonicken_approach_x     = int(sam_base_x + dir_x * t_app)
                sonicken_approach_img_h = max(1, int(sam_base_img_h + (sword_target_img_h - sam_base_img_h) * t_app))
                sonicken_warp_x         = int(sam_base_x + dir_x * t_warp)
                sonicken_warp_img_h     = max(1, int(sam_base_img_h + (sword_target_img_h - sam_base_img_h) * t_warp))
                if battle_samurai_attack_phase == BATTLE_ATTACK_PHASE_APPROACH:
                    raw_t = (battle_samurai_attack_frame / BATTLE_ATTACK_APPROACH_FRAMES) if BATTLE_ATTACK_APPROACH_FRAMES > 0 else 1.0
                    t = 1.0 - (1.0 - raw_t) ** 2
                    sam_x        = int(sam_base_x + (sonicken_approach_x - sam_base_x) * t)
                    sam_bottom_y = int(sam_base_bottom_y + (sonicken_approach_bottom_y - sam_base_bottom_y) * t)
                    sam_img_h    = max(1, int(sam_base_img_h + (sonicken_approach_img_h - sam_base_img_h) * t))
                elif battle_samurai_attack_phase == BATTLE_ATTACK_PHASE_RETURN:  # RETURN：ワープ位置→元位置
                    raw_t = (battle_samurai_attack_frame / BATTLE_ATTACK_APPROACH_FRAMES) if BATTLE_ATTACK_APPROACH_FRAMES > 0 else 1.0
                    t = 1.0 - (1.0 - raw_t) ** 2
                    sam_x        = int(sonicken_warp_x + (sam_base_x - sonicken_warp_x) * t)
                    sam_bottom_y = int(sonicken_warp_bottom_y + (sam_base_bottom_y - sonicken_warp_bottom_y) * t)
                    sam_img_h    = max(1, int(sonicken_warp_img_h + (sam_base_img_h - sonicken_warp_img_h) * t))
                else:  # FLASH（斬撃フェーズ）：battle_samurai_attack_frameは0で固定のためRETURN補間は使えない。元位置に固定する
                    sam_x        = sam_base_x
            ...
            # ④ 待機モーション：踊り子と同じ縦横逆位相スクワッシュ（剣・音速剣攻撃中は適用しない）
            sam_final_w, sam_final_h = sam_img_w, sam_img_h
            if idle_active and not sword_active and not sonicken_active:
            ...
            # ★ マカダンス中：男性の仲間（サムライ）全体へのバフ演出として、サムライは踊り子のように消さず、
            # ピンク色（ダンス中のウィンドウ色と同じ色）に点滅させる
            if dance_active and sam_img is not None:
                blink_half = max(1, BATTLE_DANCE_BLINK_PERIOD_FRAMES // 2)
                if (battle_dance_frame // blink_half) % 2 == 0:
                    sam_img.fill(BATTLE_DANCE_SAMURAI_FLASH_COLOR, special_flags=pygame.BLEND_RGB_ADD)

            # ★ 残像（音速剣の接近・後退中のみ）
            sonicken_moving = sonicken_active and battle_samurai_attack_phase in (BATTLE_ATTACK_PHASE_APPROACH, BATTLE_ATTACK_PHASE_RETURN)
            sam_current_pos = (sam_x, sam_bottom_y, sam_img_h)
            sam_trail_history_size = max(BATTLE_ATTACK_TRAIL_OFFSET_1, BATTLE_ATTACK_TRAIL_OFFSET_2)

            def find_sam_trail_pos(offset):
                if offset <= 0 or len(samurai_attack_trail) < offset:
                    return None
                px, py, _ = samurai_attack_trail[-offset]
                if (px - sam_x) ** 2 + (py - sam_bottom_y) ** 2 < BATTLE_ATTACK_TRAIL_MIN_OFFSET_PX ** 2:
                    return None
                return samurai_attack_trail[-offset]

            sam_trail_a = find_sam_trail_pos(BATTLE_ATTACK_TRAIL_OFFSET_1) if sonicken_moving else None
            sam_trail_b = find_sam_trail_pos(BATTLE_ATTACK_TRAIL_OFFSET_2) if sonicken_moving else None

            # ★ サムライの描画本体（踊り子との前後関係を切り替えられるよう、関数化して呼び出しタイミングを後で決める）
            def draw_samurai_sprite():
                if -2 not in battle_party:
                    return
                screen.set_clip(clip_rect)
                if sonicken_active and battle_samurai_attack_phase == BATTLE_ATTACK_PHASE_RETURN:
                    # 戻るとき：B → A → 現在画像 の順で重ねる
                    if sam_trail_b is not None:
                        blit_dancer_trail_image(sam_back_img_raw, sam_trail_b, BATTLE_ATTACK_TRAIL_ALPHA_2, clip_rect)
                    if sam_trail_a is not None:
                        blit_dancer_trail_image(sam_back_img_raw, sam_trail_a, BATTLE_ATTACK_TRAIL_ALPHA_1, clip_rect)
            ...
            # 残像履歴を更新する
            if sonicken_moving:
                sam_trail_key = (battle_samurai_attack_phase, battle_samurai_attack_frame)
                if samurai_attack_trail_key != sam_trail_key:
                    samurai_attack_trail_key = sam_trail_key
                    samurai_attack_trail.append(sam_current_pos)
                    if len(samurai_attack_trail) > sam_trail_history_size:
                        samurai_attack_trail.pop(0)
            else:
                samurai_attack_trail.clear()
                samurai_attack_trail_key = None

            # ★ 女戦士（仲間キャラ）の後ろ姿：踊り子の2m右を基準位置とし、斧（近接攻撃）の番には敵に接近・後退する
            if warrior_back_img_raw:
                war_orig_w, war_orig_h = warrior_back_img_raw.get_size()

                # ★ 斧（近接攻撃）：敵に接近 → 最接近で停止 → 元の位置へ後退（流れは踊り子のムチと同じ）
                ...
                if axe_active:
                    if battle_warrior_attack_phase == BATTLE_ATTACK_PHASE_APPROACH:
                        raw_t = (battle_warrior_attack_frame / BATTLE_ATTACK_APPROACH_FRAMES) if BATTLE_ATTACK_APPROACH_FRAMES > 0 else 1.0
                        axe_t = 1.0 - (1.0 - raw_t) ** 2          # 敵に近づくほど減速
                    elif battle_warrior_attack_phase in (BATTLE_ATTACK_PHASE_DAMAGE_WAIT, BATTLE_ATTACK_PHASE_FLASH):
                        axe_t = 1.0
                    else:  # BATTLE_ATTACK_PHASE_RETURN
                        raw_t = (battle_warrior_attack_frame / BATTLE_ATTACK_APPROACH_FRAMES) if BATTLE_ATTACK_APPROACH_FRAMES > 0 else 1.0
                ...
                # ④ 待機モーション：踊り子と同じ縦横逆位相スクワッシュ（斧攻撃中は適用しない）
                ...
                if sister_back_img_raw:
                    ...
                    if sister_active:
                        if battle_sister_attack_phase == BATTLE_ATTACK_PHASE_APPROACH:
                            raw_t = (battle_sister_attack_frame / BATTLE_ATTACK_APPROACH_FRAMES) if BATTLE_ATTACK_APPROACH_FRAMES > 0 else 1.0
                            sis_t = 1.0 - (1.0 - raw_t) ** 2
                        elif battle_sister_attack_phase in (BATTLE_ATTACK_PHASE_DAMAGE_WAIT, BATTLE_ATTACK_PHASE_FLASH):
                            sis_t = 1.0
                        else:
                            raw_t = (battle_sister_attack_frame / BATTLE_ATTACK_APPROACH_FRAMES) if BATTLE_ATTACK_APPROACH_FRAMES > 0 else 1.0
                            sis_t = (1.0 - raw_t) ** 2
                ...
                if kunoichi_back_img_raw:
                    ...
                    if kunoichi_active:
                        if battle_kunoichi_attack_phase == BATTLE_ATTACK_PHASE_APPROACH:
                            raw_t = (battle_kunoichi_attack_frame / BATTLE_ATTACK_APPROACH_FRAMES) if BATTLE_ATTACK_APPROACH_FRAMES > 0 else 1.0
                            kun_t = 1.0 - (1.0 - raw_t) ** 2
                        elif battle_kunoichi_attack_phase in (BATTLE_ATTACK_PHASE_DAMAGE_WAIT, BATTLE_ATTACK_PHASE_FLASH):
                            kun_t = 1.0
                        else:
                            raw_t = (battle_kunoichi_attack_frame / BATTLE_ATTACK_APPROACH_FRAMES) if BATTLE_ATTACK_APPROACH_FRAMES > 0 else 1.0
                            kun_t = (1.0 - raw_t) ** 2
                ...
                if wizard_back_img_raw:
                    ...
                    if wizard_active:
                        if battle_wizard_attack_phase == BATTLE_ATTACK_PHASE_APPROACH:
                            raw_t = (battle_wizard_attack_frame / BATTLE_ATTACK_APPROACH_FRAMES) if BATTLE_ATTACK_APPROACH_FRAMES > 0 else 1.0
                            wiz_t = 1.0 - (1.0 - raw_t) ** 2
                        elif battle_wizard_attack_phase in (BATTLE_ATTACK_PHASE_DAMAGE_WAIT, BATTLE_ATTACK_PHASE_FLASH):
                            wiz_t = 1.0
                        else:
                            raw_t = (battle_wizard_attack_frame / BATTLE_ATTACK_APPROACH_FRAMES) if BATTLE_ATTACK_APPROACH_FRAMES > 0 else 1.0
                            wiz_t = (1.0 - raw_t) ** 2
                ...
                if fighter_back_img_raw:
                    ...
                    if fighter_active:
                        if battle_fighter_attack_phase == BATTLE_ATTACK_PHASE_APPROACH:
                            raw_t = (battle_fighter_attack_frame / BATTLE_ATTACK_APPROACH_FRAMES) if BATTLE_ATTACK_APPROACH_FRAMES > 0 else 1.0
                            fig_t = 1.0 - (1.0 - raw_t) ** 2
                        elif battle_fighter_attack_phase in (BATTLE_ATTACK_PHASE_DAMAGE_WAIT, BATTLE_ATTACK_PHASE_FLASH):
                            fig_t = 1.0
                        else:
                            raw_t = (battle_fighter_attack_frame / BATTLE_ATTACK_APPROACH_FRAMES) if BATTLE_ATTACK_APPROACH_FRAMES > 0 else 1.0
                            fig_t = (1.0 - raw_t) ** 2
                ...
                # ★ 攻撃中の味方キャラ（踊り子:whip_active／サムライ:samurai_active／女戦士:warrior_active等）を、
                # 他のキャラより奥（先）に描画することで、攻撃のための移動時に手前のキャラと重なって不自然にならないようにする
                # （マカダンス中は踊り子が画面下に消えているため、サムライ・その他のみ描画する）
                def _draw_non_attacker_sprites(skip_fn):
                    for fn in [draw_dancer_sprite, draw_samurai_sprite, draw_warrior_sprite,
                ...
                # ★ 攻撃中の味方キャラ（踊り子の場合はwhip_active、サムライの場合はsamurai_active）を、
                # もう一方のキャラより奥（先）に描画することで、攻撃のための移動時に手前のキャラと重なって不自然にならないようにする
                # （マカダンス中は踊り子が画面下に消えているため、サムライのみ描画する）
                if dance_active:
                    draw_samurai_sprite()
                elif samurai_active:
                    draw_samurai_sprite()
    ...
    # ★ 心眼剣の斬撃エフェクト：画面左端→右端の水平1本（暗転中も視認できるよう手前に描画）
    if 0 <= battle_shingan_target_enemy_index < len(enemy_rects) and shingan_target_top_y is not None:
        screen.set_clip(clip_rect)
        for slash_i in range(SHINGANKEN_SLASH_COUNT):
            if battle_shingan_slash_frames[slash_i] < SHINGANKEN_SLASH_TOTAL_FRAMES:
                draw_shinganken_slash(enemy_rects[battle_shingan_target_enemy_index], shingan_target_top_y, battle_shingan_slash_frames[slash_i])
        screen.set_clip(None)

    # ★ 音速剣の斬撃エフェクト：心眼剣と同じ draw_shingan_slash を使用（暗転オーバーレイはなし）
    if 0 <= battle_sonicken_target_enemy_index < len(enemy_rects) and sonicken_target_top_y is not None:
        screen.set_clip(clip_rect)
        for slash_i in range(SONICKEN_SLASH_COUNT):
            if battle_sonicken_slash_frames[slash_i] < BATTLE_SLASH_TOTAL_FRAMES:
                draw_shingan_slash(enemy_rects[battle_sonicken_target_enemy_index], sonicken_target_top_y, battle_sonicken_slash_frames[slash_i], battle_sonicken_slash_angles[slash_i])
        screen.set_clip(None)
...
def render_battle_menu(options, selected_index, chara_label=''):
    ...
    # 半透明黒の背景（角丸）
    bg = pygame.Surface((BATTLE_MENU_WIDTH, menu_height), pygame.SRCALPHA)
    pygame.draw.rect(
    ...
    pygame.draw.rect(
        ...
        pygame.Rect(menu_x, menu_y, BATTLE_MENU_WIDTH, menu_height),
...
def render_result():
    ...
    if battle_flashout_frame < BATTLE_FLASHOUT_FRAMES:
        ...
        _ki = result_flashout_killed_enemy_index
        if 0 <= _ki < len(ENEMY_X_RATIOS) and enemy_img_raw:
            # トドメを刺した対象敵のみ描画してから白フェードオーバーレイをかける
            # → 対象敵がHP点滅しつつ白に溶けていく演出（打倒済の他の敵は表示しない）
            bg_surf = pygame.Surface((SCREEN_W, BATTLE_MAIN_WINDOW_HEIGHT))
            bg_surf.fill(BATTLE_WINDOW_COLOR)
            screen.blit(bg_surf, (0, band_y))
            ...
            _raw = apply_damage_flash(_raw, "goblin_idle.png", ENEMY_GOBLIN_HEIGHT_M, enemy_damage_anim_old_hp[_ki], enemy_damage_anim_new_hp[_ki], GOBLIN_MAX_HP, enemy_damage_anim_frame[_ki], enemy_damage_anim_flash_color[_ki])
            _eimg, _epos, _ = smoothscale_visible(_raw, _eiw, _eih, (_ex, _eby), _clip)
            if _eimg is not None:
                screen.blit(_eimg, _epos)
            screen.set_clip(None)
            white_ov = pygame.Surface((SCREEN_W, BATTLE_MAIN_WINDOW_HEIGHT))
            white_ov.fill((255, 255, 255))
            white_ov.set_alpha(int(255 * t))
            screen.blit(white_ov, (0, band_y))
        else:
            # 対象敵なし（炎・デバッグキーなど）：バトルウィンドウ色→白へ単純フェード
            ...
            overlay.fill(fc)
            screen.blit(overlay, (0, band_y))
        # トドメを刺した味方（通常は踊り子、サムライがトドメを刺した場合はサムライ）の後ろ姿をフェードアウト
        # （result_flashout_dancer_override / result_flashout_samurai_override が指定されている場合は、
    ...
    if result_active_win_img:
        ...
        if result_flashout_is_warrior:
            ...
            # 踊り子（bunny_front.png）：画像上端からRESULT_DANCER_WIN_TOP_MARGIN_M分の位置が
            # ゲームウィンドウ上端(band_y)に一致するように配置する
            # （initialize()で一度だけ行ったスケールのまま、追加の拡縮は行わない）
            win_h = result_active_win_img.get_height()
            top_margin_px = int(win_h * RESULT_DANCER_WIN_TOP_MARGIN_M / DANCER_HEIGHT_M) if DANCER_HEIGHT_M > 0 else 0
            img_rect = result_active_win_img.get_rect(midtop=(cx, band_y - top_margin_px))
