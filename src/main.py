...
SISTER_HEIGHT_M  = 1.6  # シスターの身長（メートル）
...
def render_status():
    ...
    # ★ ヒロイン・サムライ・女戦士・シスターを身長(STATUS_*_HEIGHT_M)に応じたサイズで、足元をウィンドウ下端に揃えて
    # 左からヒロイン・サムライ・女戦士・シスターの順に、4人均等配置で表示する
    # （STATUS_METER_TO_PIXEL：1mあたりのピクセル数。STATUS_CHARACTER_SPACING_M：隣り合うキャラの足元中心の横方向の間隔）
    # 4人の配置：cx - 1.5s, cx - 0.5s, cx + 0.5s, cx + 1.5s（中央対称）
    cx = SCREEN_W // 2
    spacing_px = int(STATUS_CHARACTER_SPACING_M * STATUS_METER_TO_PIXEL)
    hs = spacing_px // 2  # spacing_px の半分（0.5s）
    ...
    if heroine_img_raw:
        ...
        img_rect = img.get_rect(midbottom=(cx - spacing_px - hs, band_bottom))
    ...
    if samurai_img_raw:
        ...
        img_rect = img.get_rect(midbottom=(cx - hs, band_bottom))
        screen.blit(img, img_rect)
        if is_debug:
            pygame.draw.rect(screen, (255, 255, 255), img_rect, 1)  # ★ [デバッグ] 元画像サイズの枠線
    ...
    if sister_img_raw:
        ...
        if is_debug:
            ...
            draw_art_top_debug_line(sister_art_key, SISTER_HEIGHT_M, band_bottom, img_h, img_rect.left, img_rect.right)  # ★ [デバッグ] 実際の絵の上端
